...
- 当前单元因不改变行为的机械性编译或静态检查问题受阻时，可以直接做最小修复并继续，包括补充或移除标准库及已锁定依赖的 import、修正变量拼写、未使用变量、作用域和明显的类型对齐问题；不得借此新增、升级或替换依赖，也不得修改接口、业务逻辑、安全契约或架构。
- 上述机械性修复必须在当前单元证据中记录阻塞现象、最小修复和验证结果，运行与修复直接相关的局部门禁，并随当前单元一并提交；一旦修复需要超出该边界，立即停止并报告。
...
- P00～P42 不因普通源码改动反复重建应用镜像；只有当前单元或执行 Plan 明确要求镜像级验证、Dockerfile / Compose / 基础镜像 / 构建参数等镜像定义或构建输入发生与当前单元相关的变化、容器集成验证必须使用新产物，或者执行 P41 / P43 时才构建镜像。
- 确需构建镜像时默认复用 BuildKit 或本地层缓存；只有验证基础镜像或工具链升级、排查缓存污染，或者执行 Plan 明确要求干净构建时才使用 `--no-cache`。
