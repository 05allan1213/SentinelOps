...
type RuntimeService struct{ Store *workflow.GORMStore }
...
func BuildBudgetDTO(limitsJSON, usageJSON, reservationsJSON *string) v1.RuntimeBudgetDTO {
	...
	r := v1.RuntimeBudgetDTO{MaxModelCalls: toInt(l.MaxModelCalls), MaxL0ToolCalls: toInt(l.MaxL0ToolCalls), MaxDurationMs: toI64(l.MaxDurationMS), ModelCalls: toInt(u.ModelCalls), ToolCalls: toInt(u.L0ToolCalls), InputTokens: toI64(u.InputTokens), OutputTokens: toI64(u.OutputTokens), CostCNY: toF(u.CostCNY), MCPCalls: toInt(u.MCPCalls), RAGCalls: toInt(u.RAGDocuments), ResourceMeta: meta}
	return r
}
...
func BuildContextDTO(s workflow.DurableContextSnapshot, revUsed, revCommitted uint64, includeHistory bool, owner string, ctx context.Context) (v1.ContextDTO, error) {
	...
	return v1.ContextDTO{Identity: identity, SessionRevisionUsed: revUsed, SessionRevisionCommitted: revCommitted, SummaryHash: hex.EncodeToString(hash[:]), HistoryCount: count, BudgetLimitsHash: hex.EncodeToString(bh[:]), DeadlineAt: &s.DeadlineAt, RuntimeCompatibilityHash: "", ResourceMeta: v1.ResourceMeta{Availability: v1.AvailabilityAvailable, DataQuality: v1.DataQualityComplete}}, nil
}
...
func BuildCompatibilityDTO(run mysql.WorkflowRun, attempt *mysql.WorkflowAttempt, worker *mysql.RuntimeWorkerSnapshot) v1.RuntimeCompatibilityDTO {
	...
	if attempt != nil && attempt.ExecutingWorkerFingerprint != nil {
