...
const (
	// DurableAgentPlan 是 phase26 后所有新 durable Run 使用的官方 Plan/Executor/Replanner 入口。
...
// DurableServiceConfig 描述 phase20 API 所需的唯一 Store primitive。
...
func (s *DurableService) CreateRun(ctx context.Context, request CreateDurableRunRequest) (*mysql.WorkflowRun, error) {
	...
	// phase14 requires the three base hard limits on every durable Run.  Keep the
