...
func TestHumanCRUDChangesDataWithoutApprovalOrEffect(t *testing.T) {
	db := p26HumanCRUDDatabase(t)
	event := dao.Event{
		ID: "p26-human-crud", Title: "P26 human CRUD", Content: "manual operation",
		EventType: "manual", Severity: "medium", Source: "p26", Status: "new",
		Metadata: `{}`, RawPayload: `{}`,
	}
	...
	ctx := policy.WithIdentity(context.Background(), policy.Identity{
		UserID: "admin-p26", Role: policy.RoleAdmin, Scope: policy.Scope{All: true},
	})
...
func p26HumanCRUDDatabase(t *testing.T) *gorm.DB {
	t.Helper()
	baseDSN := os.Getenv("SENTINELOPS_TEST_DSN")
	if baseDSN == "" {
		t.Fatal("SENTINELOPS_TEST_DSN is required for P26 human CRUD integration")
	}
	config, err := driver.ParseDSN(baseDSN)
	if err != nil {
		t.Fatalf("parse P26 test DSN: %v", err)
	}
	if config.DBName != "sentinelops_p03" {
		t.Fatalf("refuse non-disposable database %q", config.DBName)
	}
	databaseName := "sentinelops_p03_p26_human_crud"
	adminConfig := *config
	...
	if gooseBinary == "" {
		...
		if err != nil {
			t.Fatal("goose v3.27.3 is required for P26 human CRUD integration")
		}
	}
