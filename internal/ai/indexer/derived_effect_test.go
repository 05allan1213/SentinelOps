...
func TestDerivedEffectPartialIndexerFailureIsReturned(t *testing.T) {
	...
	ids, err := finalizeStoreResult([]string{"event-phase24-1"}, 2, []error{partial})
	if len(ids) != 1 || !errors.Is(err, partial) {
		t.Fatalf("ids=%v err=%v", ids, err)
	}
	ids, err = finalizeStoreResult([]string{"event-phase24-1"}, 2, nil)
	if len(ids) != 1 || err == nil {
		t.Fatalf("silent partial result ids=%v err=%v", ids, err)
	}
	ids, err = finalizeStoreResult([]string{"event-phase24-1", "event-phase24-2"}, 2, nil)
