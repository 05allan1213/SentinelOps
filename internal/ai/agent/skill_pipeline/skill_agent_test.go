...
type fixture34Backend struct {
	matters []skill.FrontMatter
	items   map[string]skill.Skill
	gets    int
}

func (b *fixture34Backend) List(context.Context) ([]skill.FrontMatter, error) {
	return append([]skill.FrontMatter(nil), b.matters...), nil
}

func (b *fixture34Backend) Get(_ context.Context, name string) (skill.Skill, error) {
	b.gets++
...
type fixture34Model struct{}

func (*fixture34Model) Generate(context.Context, []*schema.Message, ...model.Option) (*schema.Message, error) {
	return schema.AssistantMessage("ok", nil), nil
}

func (*fixture34Model) Stream(context.Context, []*schema.Message, ...model.Option) (*schema.StreamReader[*schema.Message], error) {
	return schema.StreamReaderFromArray([]*schema.Message{schema.AssistantMessage("ok", nil)}), nil
}

func (*fixture34Model) BindTools([]*schema.ToolInfo) error { return nil }

func TestSkillBackendListGetAndValidatorLoadLegalSkill(t *testing.T) {
	backend := &fixture34Backend{
...
func TestSkillValidatorRejectsDuplicatePathDescriptionSizeAndOverrides(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := ValidateBackend(context.Background(), &fixture34Backend{matters: matters, items: items}, ValidationPolicy{BaseDir: "/skills", MaxBytes: 10})
...
func TestSkillSnapshotsChangeWithContent(t *testing.T) {
	first := &fixture34Backend{matters: []skill.FrontMatter{{Name: "summary", Description: "summary"}}, items: map[string]skill.Skill{"summary": {FrontMatter: skill.FrontMatter{Name: "summary", Description: "summary"}, Content: "one", BaseDirectory: "/skills/summary"}}}
	second := &fixture34Backend{matters: []skill.FrontMatter{{Name: "summary", Description: "summary"}}, items: map[string]skill.Skill{"summary": {FrontMatter: skill.FrontMatter{Name: "summary", Description: "summary"}, Content: "two", BaseDirectory: "/skills/summary"}}}
