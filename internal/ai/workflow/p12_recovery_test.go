...
func TestRecoverySelectorFactsComeOnlyFromDurableTruth(t *testing.T) {
	db := newP07Database(t, "p12_facts")
	store, token := p09ClaimRun(t, db, "p12-facts", time.Hour)
	ctx := p12WorkerContext(t, token, "user-p09-p12-facts")
	...
	p09InsertGuardedTruthFixtures(t, db, token.RunID, token.Generation)
	if err := db.Table("agent_approvals").Where("id = ?", "p09-approval").Updates(map[string]any{
...
func TestRecoverySelectorPersistsFencedResumeReplayAndParkedEvents(t *testing.T) {
	db := newP07Database(t, "p12_events")
	store, token := p09ClaimRun(t, db, "p12-events", time.Hour)
	ctx := p12WorkerContext(t, token, "user-p09-p12-events")
...
func TestRecoverySelectorRuntimeRestoreRequiresExactHashAndValidDependency(t *testing.T) {
	db := newP07Database(t, "p12_restore")
	store, token := p09ClaimRun(t, db, "p12-restore", time.Hour)
	ctx := p12WorkerContext(t, token, "user-p09-p12-restore")
	var run mysql.WorkflowRun
	if err := db.First(&run, "id = ?", token.RunID).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Exec(`INSERT INTO agent_approvals
		...
		VALUES ('p12-restore-approval', ?, 'tool', 'v1', ?, 'L1', JSON_OBJECT('safe', TRUE), ?, ?, ?,
		 'user', 'pending', 1, ?, ?, ?)`, token.RunID, p09Hash("p12-schema"), p09Hash("p12-proposal"),
		p09Hash("p12-policy"), *run.RuntimeCompatibilityHash, now, now, now).Error; err != nil {
		t.Fatal(err)
	}
	if err := store.RestoreRuntimeCompatible(ctx, RecoveryRestoreInput{
		Lease: token, CurrentCompatibilityHash: *run.RuntimeCompatibilityHash, Mode: RecoveryModeReplay,
	...
	if err := db.Exec("DELETE FROM agent_approvals WHERE id = 'p12-restore-approval'").Error; err != nil {
		t.Fatal(err)
	}
	if err := store.RestoreRuntimeCompatible(ctx, RecoveryRestoreInput{
		Lease: token, CurrentCompatibilityHash: *run.RuntimeCompatibilityHash, Mode: RecoveryModeReplay,
...
func TestRecursiveCancelHandoffRequiresCurrentGenerationCheckpoint(t *testing.T) {
	db := newP07Database(t, "p12_handoff")
	store, token := p09ClaimRun(t, db, "p12-handoff", time.Hour)
	ctx := p12WorkerContext(t, token, "user-p09-p12-handoff")
	...
	if err := store.Set(ctx, p10CheckpointID(t, token.RunID), payload); err != nil {
		t.Fatal(err)
	}
	if err := store.RequireCommittedCheckpoint(ctx, token); err != nil {
	...
	if err := store.Set(ctx, p10CheckpointID(t, token.RunID), []byte("stale")); err == nil {
		t.Fatal("lost lease wrote checkpoint after immediate cancel boundary")
	}
}

func p12WorkerContext(t *testing.T, token LeaseToken, userID string) context.Context {
	t.Helper()
	ctx, err := ContextWithLeaseToken(p08UserContext(userID), token)
	if err != nil {
		t.Fatal(err)
	}
	return ctx
