...
func TestDerivedEffectRowsKeepStableParentAndIndependentKeys(t *testing.T) {
	db := newP07Database(t, "phase24_derived_identity")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-derived", "block_ip")
...
func TestExternalEffectRejectsUnredactedSecretBeforeLedger(t *testing.T) {
	db := newP07Database(t, "phase24_unredacted_secret")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-secret", "webhook_out")
	input := fixture24ExternalInput(run, lease, approval)
	input.RequestRedacted = `{"password":"plaintext-phase24"}`
...
func TestExternalEffectSucceededIsReusedAfterToolReturnCrash(t *testing.T) {
	db := newP07Database(t, "phase24_succeeded_reuse")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-reuse", "webhook_out")
	input := fixture24ExternalInput(run, lease, approval)
	if _, err := store.EnsureExternalEffectDAG(ctx, input); err != nil {
		t.Fatal(err)
	}
	...
	committed, err := store.FinishExternalEffectSucceeded(ctx, FinishExternalEffectInput{
		...
		ResponseRedacted: `{"status_code":"200"}`, ExternalReference: "provider-phase24-reuse",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if !reused.Reused || reused.Effect.ID != committed.Effect.ID || reused.Response != committed.Response || reused.Effect.ExternalReference == nil || *reused.Effect.ExternalReference != "provider-phase24-reuse" {
		t.Fatalf("committed=%#v reused=%#v", committed, reused)
	}
}

func TestDerivedEffectTransactionalPrimaryCanStartMilvusChild(t *testing.T) {
	db := newP07Database(t, "phase24_transactional_derived")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-transactional-derived", "save_intelligence")
	primaryInput := fixture23EffectInput(run, lease, approval, "call-phase24-transactional")
	...
	if _, err := store.TransitionEffectWithEvent(ctx, primaryInput, func(context.Context) (string, error) {
		return `{"id":"intel-phase24-derived"}`, nil
	}); err != nil {
		t.Fatal(err)
	}
...
func TestEffectDeadlineIsBoundedByCurrentLease(t *testing.T) {
	db := newP07Database(t, "phase24_deadline")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-deadline", "block_ip")
	input := fixture24ExternalInput(run, lease, approval)
	if _, err := store.EnsureExternalEffectDAG(ctx, input); err != nil {
		t.Fatal(err)
	}
...
func TestUnknownWindowParksRunAndKeepsSessionLockAtomically(t *testing.T) {
	db := newP07Database(t, "phase24_unknown_window")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-unknown", "webhook_out")
	input := fixture24ExternalInput(run, lease, approval)
	if _, err := store.EnsureExternalEffectDAG(ctx, input); err != nil {
		t.Fatal(err)
	}
...
func TestUnknownWindowAfterSucceededEndpointLedgerFailureParksRun(t *testing.T) {
	db := newP07Database(t, "phase24_success_commit_window")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-success-window", "webhook_out")
	input := fixture24ExternalInput(run, lease, approval)
	if _, err := store.EnsureExternalEffectDAG(ctx, input); err != nil {
		t.Fatal(err)
	}
...
func TestDerivedEffectPartialSuccessCannotBecomeOverallSuccess(t *testing.T) {
	db := newP07Database(t, "phase24_partial_derived")
	store, ctx, run, lease, approval := fixture23ApprovedEffectFixture(t, db, "phase24-partial", "block_ip")
	input := fixture24ExternalInput(run, lease, approval)
	if _, err := store.EnsureExternalEffectDAG(ctx, input); err != nil {
		t.Fatal(err)
	}
...
func fixture24ExternalInput(run *mysql.WorkflowRun, lease LeaseToken, approval *mysql.AgentApproval) ExternalEffectExecutionInput {
	...
	return ExternalEffectExecutionInput{
		...
		ToolCallIDObserved: "call-phase24", ToolName: approval.ToolName, ToolRevision: approval.ToolRevision,
		...
		Attempt: run.Attempt, TraceID: "trace-phase24-" + run.ID, GateAllowed: true, Derived: steps,
	}
}
