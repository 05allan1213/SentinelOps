...
const p07DisposableDatabasePrefix = "sentinelops_p03"
...
func newP07Database(t *testing.T, suffix string) *gorm.DB {
	...
	if cfg.DBName != p07DisposableDatabasePrefix {
		t.Fatalf("refuse non-disposable database %q; want %q", cfg.DBName, p07DisposableDatabasePrefix)
	}
	if !regexp.MustCompile(`^[a-z0-9_]+$`).MatchString(suffix) {
		t.Fatalf("unsafe disposable database suffix %q", suffix)
	}

	databaseName := p07DisposableDatabasePrefix + "_" + suffix
	adminCfg := *cfg
