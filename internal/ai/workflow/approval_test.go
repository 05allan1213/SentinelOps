...
func TestApprovalStableIdentityAndRunProposalUnique(t *testing.T) {
	db := newP07Database(t, "phase21_identity_unique")
	proposalHash := strings.Repeat("1", 64)
	approvalID, err := policy.ApprovalID("run-phase21-unique", proposalHash)
	if err != nil {
		t.Fatal(err)
	}
	first := fixture21ApprovalRow(approvalID, "run-phase21-unique", proposalHash, "requester", ApprovalStatusPreparing, time.Now().Add(time.Hour))
	if err := db.Create(&first).Error; err != nil {
		t.Fatalf("create stable Approval: %v", err)
	}
	duplicate := fixture21ApprovalRow("different-id", first.RunID, proposalHash, "requester", ApprovalStatusPreparing, time.Now().Add(time.Hour))
...
func TestApprovalDecisionCASIsIdempotentAndWakesRun(t *testing.T) {
	db := newP07Database(t, "phase21_decision_idempotent")
	store, approval := fixture21SeedApproval(t, db, "idempotent", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
	ctx := fixture21Identity("approver", policy.RoleApprover)
...
func TestApprovalDecisionParksRunWhenAttemptsExhausted(t *testing.T) {
	db := newP07Database(t, "phase21_attempts_exhausted")
	store, approval := fixture21SeedApproval(t, db, "attempts-exhausted", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
...
func TestApprovalDecisionExpiredReturnsStableConflict(t *testing.T) {
	db := newP07Database(t, "phase21_expired")
	store, approval := fixture21SeedApproval(t, db, "expired", ApprovalStatusPending, "requester", time.Now().Add(-time.Minute))
	ctx := fixture21Identity("approver", policy.RoleApprover)
...
func TestPreparingApprovalIsInvisibleAndUndecidable(t *testing.T) {
	db := newP07Database(t, "phase21_preparing")
	store, approval := fixture21SeedApproval(t, db, "preparing", ApprovalStatusPreparing, "requester", time.Now().Add(time.Hour))
	ctx := fixture21Identity("approver", policy.RoleApprover)
...
func TestApprovalListAndDetailReturnOnlyActionableOthers(t *testing.T) {
	db := newP07Database(t, "phase21_list_detail")
	store, other := fixture21SeedApproval(t, db, "list-other", ApprovalStatusPending, "other", time.Now().Add(time.Hour))
	fixture21SeedApproval(t, db, "list-self", ApprovalStatusPending, "approver", time.Now().Add(time.Hour))
	fixture21SeedApproval(t, db, "list-preparing", ApprovalStatusPreparing, "other-two", time.Now().Add(time.Hour))
	ctx := fixture21Identity("approver", policy.RoleApprover)
	items, err := store.ListPendingApprovals(ctx, 20)
...
func TestApprovalRoleAndSelfApprovalAreRejected(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			db := newP07Database(t, "phase21_role_"+strings.ToLower(test.name))
			store, approval := fixture21SeedApproval(t, db, "role-"+test.name, ApprovalStatusPending, test.requester, time.Now().Add(time.Hour))
			_, err := store.DecideApprovalAndWakeRun(fixture21Identity(test.actorID, test.role), DecideApprovalInput{
				ApprovalID: approval.ID, ProposalHash: approval.ProposalHash,
				ExpectedVersion: approval.Version, Decision: ApprovalStatusApproved,
			})
	...
	for _, role := range []policy.Role{policy.RoleApprover, policy.RoleAdmin} {
		t.Run("other_"+string(role), func(t *testing.T) {
			db := newP07Database(t, "phase21_other_"+string(role))
			store, approval := fixture21SeedApproval(t, db, "other-"+string(role), ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
			if _, err := store.DecideApprovalAndWakeRun(fixture21Identity("reviewer-"+string(role), role), DecideApprovalInput{
				ApprovalID: approval.ID, ProposalHash: approval.ProposalHash,
...
func TestApprovalDecisionRejectsProposalHashMismatch(t *testing.T) {
	db := newP07Database(t, "phase21_hash_mismatch")
	store, approval := fixture21SeedApproval(t, db, "hash-mismatch", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
	_, err := store.DecideApprovalAndWakeRun(fixture21Identity("approver", policy.RoleApprover), DecideApprovalInput{
...
func TestApprovalDecisionRejectsUnstableIdentity(t *testing.T) {
	db := newP07Database(t, "phase21_unstable_identity")
	store, approval := fixture21SeedApproval(t, db, "unstable-identity", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
...
func TestApprovalTerminalStatusesAreImmutable(t *testing.T) {
	for _, terminal := range []string{ApprovalStatusRejected, ApprovalStatusInvalidated} {
		t.Run(terminal, func(t *testing.T) {
			db := newP07Database(t, "phase21_terminal_"+terminal)
			store, approval := fixture21SeedApproval(t, db, "terminal-"+terminal, terminal, "requester", time.Now().Add(time.Hour))
			_, err := store.DecideApprovalAndWakeRun(fixture21Identity("approver", policy.RoleApprover), DecideApprovalInput{
				ApprovalID: approval.ID, ProposalHash: approval.ProposalHash,
				ExpectedVersion: approval.Version, Decision: ApprovalStatusApproved,
			})
...
func TestApprovalAuthDisabledIsReadOnly(t *testing.T) {
	db := newP07Database(t, "phase21_auth_disabled")
	store, approval := fixture21SeedApproval(t, db, "auth-disabled", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
...
func TestApprovalDecisionReasonUsesUnifiedRedactor(t *testing.T) {
	db := newP07Database(t, "phase21_reason_redaction")
	store, approval := fixture21SeedApproval(t, db, "reason-redaction", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
	decided, err := store.DecideApprovalAndWakeRun(fixture21Identity("approver", policy.RoleApprover), DecideApprovalInput{
		ApprovalID: approval.ID, ProposalHash: approval.ProposalHash,
		ExpectedVersion: approval.Version, Decision: ApprovalStatusRejected,
...
func TestApprovalDecisionTransactionRollsBackEveryWrite(t *testing.T) {
	db := newP07Database(t, "phase21_atomic")
	store, approval := fixture21SeedApproval(t, db, "atomic", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
	if err := db.Exec(`CREATE TRIGGER reject_phase21_event BEFORE INSERT ON workflow_events
...
func TestApprovalConcurrentDecisionCASHasOneEvent(t *testing.T) {
	db := newP07Database(t, "phase21_concurrent")
	store, approval := fixture21SeedApproval(t, db, "concurrent", ApprovalStatusPending, "requester", time.Now().Add(time.Hour))
	ctx := fixture21Identity("approver", policy.RoleApprover)
...
func fixture21SeedApproval(t *testing.T, db *gorm.DB, suffix, status, requestedBy string, expiresAt time.Time) (*GORMStore, mysql.AgentApproval) {
	t.Helper()
	store := NewGORMStore(db)
	ownerCtx := fixture08UserContext(requestedBy)
	runID := "run-phase21-" + suffix
	run, err := store.CreateRunWithSessionLock(ownerCtx, fixture08CreateInput(runID, "session-phase21-"+suffix))
	if err != nil {
		t.Fatalf("create Approval fixture Run: %v", err)
	}
	fixture08MoveToRunning(t, store, ownerCtx, run.ID)
	if status != ApprovalStatusPreparing {
		if err := store.TransitionRunWithEvent(ownerCtx, RunTransition{
			...
			Lease: fixture08CurrentLease(t, db, run.ID), Event: WorkflowEventInput{Type: EventApprovalRequested},
		}); err != nil {
	...
	approval := fixture21ApprovalRow(approvalID, run.ID, proposalHash, requestedBy, status, expiresAt)
...
func fixture21ApprovalRow(id, runID, proposalHash, requestedBy, status string, expiresAt time.Time) mysql.AgentApproval {
...
func fixture21Identity(userID string, role policy.Role) context.Context {
