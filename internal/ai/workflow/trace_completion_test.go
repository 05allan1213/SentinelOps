...
func TestTraceIncompleteEventPrecedesRunCompleted(t *testing.T) {
	events, err := completionTraceEvents(CompleteRunInput{TargetStatus: RunStatusSucceeded, TraceQuality: TraceQualityIncomplete, TraceID: "trace-phase35"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTraceFlushedEventPrecedesRunCompleted(t *testing.T) {
	events, err := completionTraceEvents(CompleteRunInput{TargetStatus: RunStatusSucceeded, TraceQuality: TraceQualityComplete, TraceID: "trace-phase35"})
	if err != nil {
		t.Fatal(err)
	}
