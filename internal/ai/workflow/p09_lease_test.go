...
func TestClaimTwoWorkersOnlyOneOwnerGeneration(t *testing.T) {
	db := newP07Database(t, "p09_claim_compete")
	store := NewGORMStore(db)
	ctx := p08UserContext("user-p09-claim")
	if _, err := store.CreateRunWithSessionLock(ctx, p08CreateInput("run-p09-claim", "session-p09-claim")); err != nil {
		t.Fatalf("create claim fixture: %v", err)
	}

	start := make(chan struct{})
	results := make(chan p09ClaimResult, 2)
	...
	for _, owner := range []string{"worker-a", "worker-b"} {
		...
		go func() {
			...
			results <- p09ClaimResult{claim: claim, ok: ok, err: err}
		}()
	}
	close(start)
	wait.Wait()
	...
	if err := db.First(&run, "id = ?", "run-p09-claim").Error; err != nil {
...
func TestClaimEventFailureRollsBackLease(t *testing.T) {
	db := newP07Database(t, "p09_claim_atomic")
	store := NewGORMStore(db)
	ctx := p08UserContext("user-p09-claim-atomic")
	if _, err := store.CreateRunWithSessionLock(ctx, p08CreateInput("run-p09-claim-atomic", "session-p09-claim-atomic")); err != nil {
		t.Fatalf("create atomic claim fixture: %v", err)
	}
	if err := db.Exec(`CREATE TRIGGER reject_p09_claim BEFORE INSERT ON workflow_events
	...
	var run mysql.WorkflowRun
	if err := db.First(&run, "id = ?", "run-p09-claim-atomic").Error; err != nil {
...
func TestHeartbeatRequiresCurrentOwnerGeneration(t *testing.T) {
	db := newP07Database(t, "p09_heartbeat")
	store, token := p09ClaimRun(t, db, "heartbeat", time.Minute)
...
func TestGenerationIncreasesAfterLeaseExpiry(t *testing.T) {
	db := newP07Database(t, "p09_generation")
	store, first := p09ClaimRun(t, db, "generation", time.Minute)
	p09ExpireLease(t, db, first.RunID)
...
func TestClaimPredicateExcludesIncompleteAndUnavailableRuns(t *testing.T) {
	db := newP07Database(t, "p09_claim_predicate")
	store := NewGORMStore(db)
	now := time.Now().Truncate(time.Millisecond)
	validJSON, version, hash := `{}`, "runtime-v1", p09Hash("a")
	future := now.Add(time.Hour)
	runs := []mysql.WorkflowRun{
		{ID: "legacy", WorkflowKey: "p09", RuntimeMode: RuntimeModeLegacy, Status: RunStatusPending, ImmutableInputJSON: &validJSON, RuntimeVersion: &version, RuntimeCompatibilityHash: &hash, AvailableAt: now, StartedAt: now},
		{ID: "null-input", WorkflowKey: "p09", RuntimeMode: RuntimeModeDurableV1, Status: RunStatusPending, RuntimeVersion: &version, RuntimeCompatibilityHash: &hash, AvailableAt: now, StartedAt: now},
		{ID: "null-version", WorkflowKey: "p09", RuntimeMode: RuntimeModeDurableV1, Status: RunStatusPending, ImmutableInputJSON: &validJSON, RuntimeCompatibilityHash: &hash, AvailableAt: now, StartedAt: now},
		{ID: "null-hash", WorkflowKey: "p09", RuntimeMode: RuntimeModeDurableV1, Status: RunStatusPending, ImmutableInputJSON: &validJSON, RuntimeVersion: &version, AvailableAt: now, StartedAt: now},
		{ID: "future", WorkflowKey: "p09", RuntimeMode: RuntimeModeDurableV1, Status: RunStatusPending, ImmutableInputJSON: &validJSON, RuntimeVersion: &version, RuntimeCompatibilityHash: &hash, AvailableAt: future, StartedAt: now},
		{ID: "eligible", WorkflowKey: "p09", RuntimeMode: RuntimeModeDurableV1, Status: RunStatusPending, ImmutableInputJSON: &validJSON, RuntimeVersion: &version, RuntimeCompatibilityHash: &hash, AvailableAt: now, StartedAt: now},
	}
...
func TestLeaseReapFencesExpiredOwner(t *testing.T) {
	db := newP07Database(t, "p09_reap")
	store, token := p09ClaimRun(t, db, "reap", time.Minute)
	p09ExpireLease(t, db, token.RunID)
...
func TestStaleGenerationCannotWriteTruth(t *testing.T) {
	db := newP07Database(t, "p09_stale_truth")
	store, stale := p09ClaimRun(t, db, "stale", time.Minute)
	...
	before := p09ReadTruth(t, db, stale.RunID)
	transitionErr := store.TransitionRunWithEvent(p08UserContext("user-p09-stale"), RunTransition{
	...
	completeErr := store.CompleteRunAndCommitSession(p08UserContext("user-p09-stale"), CompleteRunInput{
	...
	after := p09ReadTruth(t, db, stale.RunID)
	if before != after {
		t.Fatalf("stale Run/Event/terminal write changed truth: before=%s after=%s", before, after)
	}

	p09InsertGuardedTruthFixtures(t, db, stale.RunID, current.Token.Generation)
	...
	err = store.withFencedRunTransaction(context.Background(), stale, RunStatusRunning, func(tx *gorm.DB, _ *mysql.WorkflowRun) error {
		...
		statements := []struct {
			...
			{`UPDATE workflow_checkpoints SET snapshot_json = JSON_OBJECT('state', 'stale') WHERE id = 'p09-checkpoint'`, nil},
			{`UPDATE agent_approvals SET decision_reason = 'stale' WHERE id = 'p09-approval'`, nil},
			{`UPDATE agent_effects SET last_error = 'stale' WHERE id = 'p09-effect'`, nil},
		}
	...
	p09AssertGuardedTruthUnchanged(t, db, stale.RunID)
}

type p09ClaimResult struct {
	claim *ClaimedRun
	ok    bool
	err   error
}

func p09ClaimRun(t *testing.T, db *gorm.DB, suffix string, duration time.Duration) (*GORMStore, LeaseToken) {
	t.Helper()
	store := NewGORMStore(db)
	ctx := p08UserContext("user-p09-" + suffix)
	if _, err := store.CreateRunWithSessionLock(ctx, p08CreateInput("run-p09-"+suffix, "session-p09-"+suffix)); err != nil {
		t.Fatalf("create P09 run: %v", err)
	}
	claim, ok, err := store.ClaimNextRun(context.Background(), ClaimInput{
		Owner: "worker-" + suffix, LeaseDuration: duration,
	})
	if err != nil || !ok {
		t.Fatalf("claim P09 run: claim=%#v ok=%v err=%v", claim, ok, err)
	}
	return store, claim.Token
}

func p09ExpireLease(t *testing.T, db *gorm.DB, runID string) {
	t.Helper()
	if err := db.Model(&mysql.WorkflowRun{}).Where("id = ?", runID).
		...
		t.Fatalf("expire P09 lease: %v", err)
	}
}

func p09ReadTruth(t *testing.T, db *gorm.DB, runID string) string {
	t.Helper()
	var run mysql.WorkflowRun
	if err := db.First(&run, "id = ?", runID).Error; err != nil {
		t.Fatalf("read P09 run truth: %v", err)
	}
	var events int64
	if err := db.Model(&mysql.WorkflowEvent{}).Where("run_id = ?", runID).Count(&events).Error; err != nil {
		t.Fatalf("count P09 events: %v", err)
	}
	return fmt.Sprintf("%s/%d/%d/%d/%v", run.Status, run.LastEventSeq, run.LeaseGeneration, events, run.FinishedAt)
}

func p09InsertGuardedTruthFixtures(t *testing.T, db *gorm.DB, runID string, generation uint64) {
	t.Helper()
	now := time.Now().Truncate(time.Millisecond)
	statements := []struct {
		query string
		...
		{`INSERT INTO workflow_checkpoints (id, run_id, checkpoint_key, snapshot_json, lease_generation, created_at, updated_at)
			VALUES ('p09-checkpoint', ?, 'p09', JSON_OBJECT('state', 'original'), ?, ?, ?)`, []any{runID, generation, now, now}},
		{`INSERT INTO agent_approvals
			...
			VALUES ('p09-approval', ?, 'tool', 'v1', ?, 'L1', JSON_OBJECT('safe', TRUE), ?, ?, ?, 'user', 'preparing', 1, ?, ?)`,
			[]any{runID, p09Hash("1"), p09Hash("2"), p09Hash("3"), p09Hash("4"), now, now}},
		{`INSERT INTO agent_effects
			...
			VALUES ('p09-effect', ?, 'p09-idempotency', 'primary', 'primary', ?, 'tool', 'v1', ?, ?,
			 'transactional_db', 'pending', 1, ?, 0, 0, ?, ?)`,
			[]any{runID, p09Hash("5"), p09Hash("6"), p09Hash("7"), generation, now, now}},
	}
	for _, statement := range statements {
...
func p09AssertGuardedTruthUnchanged(t *testing.T, db *gorm.DB, runID string) {
	t.Helper()
	var budget string
	...
	if err := db.Table("workflow_checkpoints").Where("id = ? AND JSON_UNQUOTE(JSON_EXTRACT(snapshot_json, '$.state')) = 'original'", "p09-checkpoint").Count(&checkpoint).Error; err != nil {
		t.Fatalf("read guarded checkpoint: %v", err)
	}
	if err := db.Table("agent_approvals").Where("id = ? AND decision_reason IS NULL", "p09-approval").Count(&approval).Error; err != nil {
		t.Fatalf("read guarded approval: %v", err)
	}
	if err := db.Table("agent_effects").Where("id = ? AND last_error IS NULL", "p09-effect").Count(&effect).Error; err != nil {
...
func p09Hash(char string) string {
	result := ""
	for len(result) < 64 {
		result += char
	}
