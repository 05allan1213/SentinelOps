...
func TestCleanupLeaseGenerationFenceAllowsOneWorker(t *testing.T) {
	db := newP07Database(t, "p36_cleanup_lease")
	...
	for _, owner := range []string{"worker-p36-a", "worker-p36-b"} {
		wg.Add(1)
		go func() {
			defer wg.Done()
...
func TestPhysicalDeleteRetentionProtectsResumeAndSeparatesPayloadFromAudit(t *testing.T) {
	db := newP07Database(t, "p36_physical_delete")
	store := NewGORMStore(db)
	now := time.Now().UTC().Truncate(time.Millisecond)
	oldPayload := now.Add(-31 * 24 * time.Hour)
	oldAudit := now.Add(-181 * 24 * time.Hour)

	terminal := p36RetentionRun("run-p36-terminal", RunStatusSucceeded, oldAudit)
	parked := p36RetentionRun("run-p36-parked", RunStatusParked, oldAudit)
	...
	if err := db.Create(&mysql.AgentApproval{
		ID: "approval-p36", RunID: terminal.ID, ToolName: "block_ip", ToolRevision: "v1",
		ToolSchemaHash: p36Hex('a'), RiskLevel: "L2", ProposalJSONRedacted: proposal,
		ProposalHash: p36Hex('b'), PolicyHash: p36Hex('c'), RuntimeCompatibilityHash: p36Hex('d'),
		RequestedBy: "user-p36", Status: ApprovalStatusApproved, Version: 1,
		PreparingAt: oldAudit, CreatedAt: oldAudit, DecidedAt: &oldAudit,
	}).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&mysql.AgentEffect{
		ID: "effect-p36", RunID: terminal.ID, IdempotencyKey: "effect-p36", EffectRole: "primary", EffectStep: "primary",
		ProposalHash: p36Hex('b'), ToolName: "block_ip", ToolRevision: "v1", ToolSchemaHash: p36Hex('a'),
		TargetHash: p36Hex('e'), RequestRedacted: &request, ResponseRedacted: &response,
...
func p36RetentionRun(id, status string, finished time.Time) mysql.WorkflowRun {
	input := `{"query":"sensitive"}`
	contextSnapshot := `{"history":"sensitive"}`
	version, compatibility := "runtime-v1", p36Hex('f')
	return mysql.WorkflowRun{
		ID: id, WorkflowKey: "p36", UserID: "user-p36", SessionID: "session-" + id,
...
func p36Hex(ch byte) string { return strings.Repeat(string(ch), 64) }
