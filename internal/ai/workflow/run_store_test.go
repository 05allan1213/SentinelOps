...
func TestCreateRunWithSessionLockIsAtomic(t *testing.T) {
	db := newP07Database(t, "phase08_create_atomic")
	store := NewGORMStore(db)
	ctx := fixture08UserContext("user-create")

	run, err := store.CreateRunWithSessionLock(ctx, fixture08CreateInput("run-create", "session-create"))
	if err != nil {
...
func TestCreateRunSameSessionReturnsConflict(t *testing.T) {
	db := newP07Database(t, "phase08_session_conflict")
	store := NewGORMStore(db)
	ctx := fixture08UserContext("user-conflict")
	if _, err := store.CreateRunWithSessionLock(ctx, fixture08CreateInput("run-first", "session-conflict")); err != nil {
		t.Fatalf("create first run: %v", err)
	}
	_, err := store.CreateRunWithSessionLock(ctx, fixture08CreateInput("run-second", "session-conflict"))
...
func TestCreateRunSameSessionConcurrentOnlyOneSucceeds(t *testing.T) {
	...
	store := NewGORMStore(db)
	ctx := fixture08UserContext("user-concurrent")
	...
	for index := range 2 {
		...
		go func() {
			defer wg.Done()
			<-start
			_, err := store.CreateRunWithSessionLock(ctx, fixture08CreateInput(fmt.Sprintf("run-concurrent-%d", index), "session-concurrent"))
			errs <- err
		}()
	}
	close(start)
...
func TestCreateRunBootstrapsRevisionZeroAtomic(t *testing.T) {
	db := newP07Database(t, "phase08_bootstrap_atomic")
	store := NewGORMStore(db)
	ctx := fixture08UserContext("user-bootstrap")
	if err := db.Exec(`CREATE TRIGGER reject_phase08_created BEFORE INSERT ON workflow_events
	...
	_, err := store.CreateRunWithSessionLock(ctx, fixture08CreateInput("run-bootstrap", "session-bootstrap"))
	if err == nil {
...
func TestTransitionRunWithEventIsAtomic(t *testing.T) {
	db := newP07Database(t, "phase08_transition_atomic")
	store, ctx, run := fixture08CreateRun(t, db, "transition")
	if err := db.Exec(`CREATE TRIGGER reject_phase08_transition BEFORE INSERT ON workflow_events
...
func TestEventSequenceAllocatedByDatabase(t *testing.T) {
	db := newP07Database(t, "phase08_event_sequence")
	store, ctx, run := fixture08CreateRun(t, db, "sequence")
	lease := fixture08CurrentLease(t, db, run.ID)
...
func TestTransitionRunEventPayloadIsRedactedAndBounded(t *testing.T) {
	db := newP07Database(t, "phase08_event_payload")
	store, ctx, run := fixture08CreateRun(t, db, "payload")
	secret := "sk-1234567890abcdefghijklmnop"
	err := store.TransitionRunWithEvent(ctx, RunTransition{
		RunID: run.ID, ExpectedStatus: RunStatusPending, TargetStatus: RunStatusRunning,
		Lease: fixture08CurrentLease(t, db, run.ID),
		Event: WorkflowEventInput{Type: EventRunClaimed, TraceID: "trace-phase08", Payload: EventPayload{Attributes: map[string]any{
			"api_key": secret,
			"note":    "safe",
		}}},
	})
	...
	if event.PayloadVersion != EventPayloadVersion || event.TraceID != "trace-phase08" || strings.Contains(event.Payload, secret) || !strings.Contains(event.Payload, "[REDACTED]") || !strings.Contains(event.Payload, "safe") {
...
func TestCompleteRunAndCommitSessionSucceededAtomic(t *testing.T) {
	db := newP07Database(t, "phase08_complete_success")
	store, ctx, run := fixture08CreateRun(t, db, "complete_success")
	...
	if err := store.CompleteRunAndCommitSession(ctx, CompleteRunInput{
		...
		Lease:         fixture08CurrentLease(t, db, run.ID),
		OutputPayload: `{"answer_ref":"trace-phase08","api_key":"` + secret + `"}`, TraceQuality: "complete", RevisionStateJSON: revisionJSON,
	}); err != nil {
	...
	if !fixture08JSONEqual(revision.StateJSON, string(revisionJSON)) {
	...
	for _, source := range []string{RunStatusPending, RunStatusRunning, RunStatusWaitingApproval, RunStatusRetryableFailed, RunStatusParked, RunStatusReconciling} {
		for _, target := range []string{RunStatusFailed, RunStatusCanceled} {
			t.Run(source+"_to_"+target, func(t *testing.T) {
				db := newP07Database(t, "phase08_complete_"+strings.ReplaceAll(source+"_"+target, "-", "_"))
				store, ctx, run := fixture08CreateRun(t, db, source+target)
				fixture08SetRunState(t, db, run.ID, source, fixture08ParkReason(source))
...
func TestCompleteRunTransactionFailureRollsBackEverything(t *testing.T) {
	db := newP07Database(t, "phase08_complete_rollback")
	store, ctx, run := fixture08CreateRun(t, db, "complete_rollback")
	fixture08MoveToRunning(t, store, ctx, run.ID)
	if err := db.Exec(`CREATE TRIGGER reject_phase08_revision BEFORE INSERT ON session_state_revisions
...
func TestLegacyFinishRunCannotTerminateDurableRun(t *testing.T) {
	db := newP07Database(t, "phase08_legacy_finish_guard")
	store, ctx, run := fixture08CreateRun(t, db, "legacy_guard")
...
func TestTransitionRunCannotCompleteDurableRun(t *testing.T) {
	db := newP07Database(t, "phase08_transition_terminal_guard")
	store, ctx, run := fixture08CreateRun(t, db, "transition_terminal_guard")
	fixture08MoveToRunning(t, store, ctx, run.ID)
	err := store.TransitionRunWithEvent(ctx, RunTransition{
		RunID: run.ID, ExpectedStatus: RunStatusRunning, TargetStatus: RunStatusSucceeded,
		Lease: fixture08CurrentLease(t, db, run.ID),
...
func fixture08UserContext(userID string) context.Context {
...
func fixture08CreateInput(runID, sessionID string) CreateRunInput {
	return CreateRunInput{
		ID:                 runID,
		WorkflowKey:        "phase08-test",
		SessionID:          sessionID,
		...
		CreatedEvent: WorkflowEventInput{
			...
			Payload: EventPayload{Attributes: map[string]any{"source": "phase08-test"}},
		},
	}
}

func fixture08CreateRun(t *testing.T, db *gorm.DB, suffix string) (*GORMStore, context.Context, *mysql.WorkflowRun) {
	t.Helper()
	store := NewGORMStore(db)
	ctx := fixture08UserContext("user-" + suffix)
	run, err := store.CreateRunWithSessionLock(ctx, fixture08CreateInput("run-"+suffix, "session-"+suffix))
	if err != nil {
		t.Fatalf("create phase08 run: %v", err)
	}
	return store, ctx, run
}

func fixture08MoveToRunning(t *testing.T, store *GORMStore, ctx context.Context, runID string) {
	t.Helper()
	if err := store.TransitionRunWithEvent(ctx, RunTransition{
		...
		Lease: fixture08CurrentLease(t, store.db, runID),
...
func fixture08CurrentLease(t *testing.T, db *gorm.DB, runID string) LeaseToken {
	...
	if run.LeaseOwner == nil || run.LeaseGeneration == 0 || run.LeaseUntil == nil || !run.LeaseUntil.After(time.Now()) {
		...
		if err := db.Model(&mysql.WorkflowRun{}).Where("id = ?", runID).Updates(map[string]any{
			...
			t.Fatalf("seed phase08 test lease: %v", err)
		}
...
func fixture08SetRunState(t *testing.T, db *gorm.DB, runID, status, parkReason string) {
	t.Helper()
...
func fixture08ParkReason(status string) string {
...
func TestCompleteRunRejectsSuccessfulRevisionForFailure(t *testing.T) {
	db := newP07Database(t, "phase08_failure_revision_reject")
	store, ctx, run := fixture08CreateRun(t, db, "failure_revision_reject")
	err := store.CompleteRunAndCommitSession(ctx, CompleteRunInput{
...
func TestCreateRunRejectsInvalidDurableContract(t *testing.T) {
	db := newP07Database(t, "phase08_invalid_contract")
	input := fixture08CreateInput("run-invalid", "session-invalid")
	input.RuntimeSnapshot.RuntimeCompatibilityHash = "short"
	_, err := NewGORMStore(db).CreateRunWithSessionLock(fixture08UserContext("user-invalid"), input)
	if err == nil {
...
func TestTransitionRunCASRejectsStaleExpectedState(t *testing.T) {
	db := newP07Database(t, "phase08_stale_cas")
	store, ctx, run := fixture08CreateRun(t, db, "stale_cas")
	err := store.TransitionRunWithEvent(ctx, RunTransition{
		RunID: run.ID, ExpectedStatus: RunStatusRunning, TargetStatus: RunStatusWaitingApproval,
		Lease: fixture08CurrentLease(t, db, run.ID),
...
func TestCompleteRunCASRejectsStaleExpectedState(t *testing.T) {
	db := newP07Database(t, "phase08_complete_stale")
	store, ctx, run := fixture08CreateRun(t, db, "complete_stale")
	err := store.CompleteRunAndCommitSession(ctx, CompleteRunInput{
...
func TestCreateRunUsesLatestSessionRevisionSnapshot(t *testing.T) {
	db := newP07Database(t, "phase08_latest_revision")
	...
	run, err := NewGORMStore(db).CreateRunWithSessionLock(fixture08UserContext("user-latest"), fixture08CreateInput("run-latest", "session-latest"))
	if err != nil {
	...
	if err := json.Unmarshal([]byte(*run.ContextSnapshotJSON), &contextSnapshot); err != nil || !fixture08JSONEqual(string(contextSnapshot.History), latest.StateJSON) {
		t.Fatalf("frozen latest history = %s, err=%v", contextSnapshot.History, err)
	}
}

func fixture08JSONEqual(left, right string) bool {
...
func TestEventEnvelopeIsStructuredJSON(t *testing.T) {
	db := newP07Database(t, "phase08_envelope")
	_, _, run := fixture08CreateRun(t, db, "envelope")
	var event mysql.WorkflowEvent
	...
	if envelope.Schema != EventEnvelopeSchema || envelope.Data["source"] != "phase08-test" {
...
func TestCompleteRunFinishedTimestampIsNotBeforeStart(t *testing.T) {
	db := newP07Database(t, "phase08_finished_time")
	store, ctx, run := fixture08CreateRun(t, db, "finished_time")
	fixture08MoveToRunning(t, store, ctx, run.ID)
	if err := store.CompleteRunAndCommitSession(ctx, CompleteRunInput{
		RunID: run.ID, ExpectedStatus: RunStatusRunning, TargetStatus: RunStatusFailed,
		Lease: fixture08CurrentLease(t, db, run.ID),
