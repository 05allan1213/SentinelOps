...
func TestTwoPhaseApprovalPublishesOnlyExactCheckpoint(t *testing.T) {
	db := newP07Database(t, "phase22_two_phase")
	store, ctx, run, lease := fixture22RunningRun(t, db, "two-phase")
	prepared := fixture22Prepare(t, store, ctx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
	...
	if err := store.Set(checkpointContext, checkpointID, []byte("opaque-phase22-checkpoint")); err != nil {
	...
	published, err := store.PublishApprovalAndWait(ctx, PublishApprovalInput{
		...
		InterruptID: "interrupt-phase22-root", InterruptAddress: "agent:root;tool:block-ip",
		CheckpointID: checkpointID, CheckpointPayloadSHA256: fingerprint.PayloadSHA256,
		CheckpointLeaseGeneration: fingerprint.LeaseGeneration, TraceID: "trace-phase22-publish",
	})
	if err != nil {
...
func TestCheckpointFingerprintMismatchNeverPublishes(t *testing.T) {
	db := newP07Database(t, "phase22_fingerprint_mismatch")
	store, ctx, run, lease := fixture22RunningRun(t, db, "fingerprint-mismatch")
	prepared := fixture22Prepare(t, store, ctx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
	checkpointID, _ := EinoCheckpointID(run.ID)
	checkpointContext, _ := ContextWithLeaseToken(ctx, lease)
	if err := store.Set(checkpointContext, checkpointID, []byte("opaque-phase22-exact")); err != nil {
		t.Fatal(err)
	}
	fingerprint, err := store.LoadCheckpointFingerprint(ctx, lease, checkpointID)
	if err != nil {
...
func TestTwoPhaseApprovalPublicationRollsBackIfEitherEventFails(t *testing.T) {
	for _, rejectedEvent := range []string{EventAgentInterrupted, EventApprovalRequested} {
		t.Run(rejectedEvent, func(t *testing.T) {
			db := newP07Database(t, "phase22_publish_rollback_"+strings.ReplaceAll(rejectedEvent, ".", "_"))
			store, ctx, run, lease := fixture22RunningRun(t, db, "publish-rollback-"+strings.ReplaceAll(rejectedEvent, ".", "-"))
			prepared := fixture22Prepare(t, store, ctx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
			checkpointID, _ := EinoCheckpointID(run.ID)
			checkpointContext, _ := ContextWithLeaseToken(ctx, lease)
			...
			trigger := fmt.Sprintf(`CREATE TRIGGER reject_phase22_publish BEFORE INSERT ON workflow_events
				FOR EACH ROW BEGIN
					IF NEW.event_type = '%s' THEN
						SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'reject phase22 publish event';
					END IF;
				END`, rejectedEvent)
			if err := db.Exec(trigger).Error; err != nil {
				t.Fatalf("create phase22 publish failure trigger: %v", err)
			}

			_, err = store.PublishApprovalAndWait(ctx, PublishApprovalInput{
				Lease: lease, ApprovalID: prepared.ID, ProposalHash: prepared.ProposalHash,
				InterruptID: "interrupt-phase22-rollback", InterruptAddress: "agent:root;tool:block-ip",
				CheckpointID: checkpointID, CheckpointPayloadSHA256: fingerprint.PayloadSHA256,
...
func TestTwoPhaseApprovalPreparingIsGenerationFencedAndNeverResetsTerminal(t *testing.T) {
	db := newP07Database(t, "phase22_prepare_fence")
	store, ctx, run, lease := fixture22RunningRun(t, db, "prepare-fence")
	prepared := fixture22Prepare(t, store, ctx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))

	stale := lease
	if err := db.Model(&mysql.WorkflowRun{}).Where("id = ?", run.ID).Updates(map[string]any{
		"lease_owner": "worker-phase22-next", "lease_generation": lease.Generation + 1,
		"lease_until": time.Now().Add(time.Hour), "heartbeat_at": time.Now(),
	}).Error; err != nil {
		t.Fatal(err)
	}
	input := fixture22PrepareInput(run, stale, prepared.ProposalHash, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
	...
	current := stale
	current.Owner = "worker-phase22-next"
	current.Generation++
	got, err := store.PrepareApproval(ctx, fixture22PrepareInput(run, current, prepared.ProposalHash, "block_ip", policy.RiskL2, time.Now().Add(time.Hour)))
	if err != nil {
...
func TestTwoPhaseApprovalIncompatiblePreparingIsInvalidatedAndParked(t *testing.T) {
	db := newP07Database(t, "phase22_prepare_incompatible")
	store, ctx, run, lease := fixture22RunningRun(t, db, "prepare-incompatible")
	prepared := fixture22Prepare(t, store, ctx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
	if err := db.Model(&mysql.AgentApproval{}).Where("id = ?", prepared.ID).Update("tool_revision", "tampered-revision").Error; err != nil {
		t.Fatal(err)
	}
	_, err := store.PrepareApproval(ctx, fixture22PrepareInput(run, lease, prepared.ProposalHash, "block_ip", policy.RiskL2, time.Now().Add(time.Hour)))
...
func TestResumeAuthorizationRequiresExplicitExactDatabaseTruth(t *testing.T) {
	db := newP07Database(t, "phase22_resume_authorization")
	store, ownerCtx, run, lease := fixture22RunningRun(t, db, "resume-authorization")
	...
	decided, err := store.DecideApprovalAndWakeRun(fixture21Identity("phase22-approver", policy.RoleApprover), DecideApprovalInput{
	...
	if err != nil {
		t.Fatalf("approve phase22 fixture: %v", err)
	}
	claimed, ok, err := store.ClaimNextRun(context.Background(), ClaimInput{Owner: "worker-phase22-resume", LeaseDuration: time.Hour})
	if err != nil || !ok {
...
func TestCheckpointFingerprintPreparingOrphanBuildsEmptyResumeTargets(t *testing.T) {
	db := newP07Database(t, "phase22_preparing_orphan")
	store, ctx, run, lease := fixture22RunningRun(t, db, "preparing-orphan")
	prepared := fixture22Prepare(t, store, ctx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
...
func TestResumeAuthorizationTerminalDecisionsDoNotReopenApproval(t *testing.T) {
	for _, decision := range []string{ApprovalStatusRejected, ApprovalStatusExpired} {
		t.Run(decision, func(t *testing.T) {
			db := newP07Database(t, "phase22_terminal_resume_"+decision)
			store, ownerCtx, run, lease := fixture22RunningRun(t, db, "terminal-resume-"+decision)
			expiresAt := time.Now().Add(time.Hour)
			if decision == ApprovalStatusExpired {
				expiresAt = time.Now().Add(-time.Minute)
			}
			prepared := fixture22Prepare(t, store, ownerCtx, run, lease, "block_ip", policy.RiskL2, expiresAt)
			fixture22Publish(t, store, ownerCtx, run.ID, lease, prepared)
			decided, err := store.DecideApprovalAndWakeRun(fixture21Identity("phase22-terminal-approver-"+decision, policy.RoleApprover), DecideApprovalInput{
				ApprovalID: prepared.ID, ProposalHash: prepared.ProposalHash, ExpectedVersion: prepared.Version, Decision: decision,
			})
			if err != nil {
				t.Fatal(err)
			}
			claimed, ok, err := store.ClaimNextRun(context.Background(), ClaimInput{Owner: "worker-phase22-terminal-" + decision, LeaseDuration: time.Hour})
			if err != nil || !ok {
			...
			refreshed, err := store.PrepareApproval(resumeCtx, fixture22PrepareInput(run, claimed.Token, prepared.ProposalHash, "block_ip", policy.RiskL2, time.Now().Add(time.Hour)))
			if err != nil {
				t.Fatal(err)
			}
			var count int64
...
func TestResumeAuthorizationClosedGateParksWithoutChangingApprovedAudit(t *testing.T) {
	db := newP07Database(t, "phase22_closed_gate")
	store, ownerCtx, run, lease := fixture22RunningRun(t, db, "closed-gate")
	prepared := fixture22Prepare(t, store, ownerCtx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
	fixture22Publish(t, store, ownerCtx, run.ID, lease, prepared)
	decided, err := store.DecideApprovalAndWakeRun(fixture21Identity("phase22-gate-approver", policy.RoleApprover), DecideApprovalInput{
	...
	claimed, ok, err := store.ClaimNextRun(context.Background(), ClaimInput{Owner: "worker-phase22-closed-gate", LeaseDuration: time.Hour})
	if err != nil || !ok {
...
func TestResumeAuthorizationRejectsL2SelfApproval(t *testing.T) {
	...
	prepared := fixture22Prepare(t, store, ownerCtx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
	fixture22Publish(t, store, ownerCtx, run.ID, lease, prepared)
	_, err := store.DecideApprovalAndWakeRun(fixture21Identity(run.UserID, policy.RoleApprover), DecideApprovalInput{
		ApprovalID: prepared.ID, ProposalHash: prepared.ProposalHash, ExpectedVersion: prepared.Version, Decision: ApprovalStatusApproved,
	})
...
func TestCheckpointFingerprintOverwriteParksWithoutChangingApprovedAudit(t *testing.T) {
	db := newP07Database(t, "phase22_overwrite_parks")
	store, ownerCtx, run, lease := fixture22RunningRun(t, db, "overwrite-parks")
	prepared := fixture22Prepare(t, store, ownerCtx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
	fixture22Publish(t, store, ownerCtx, run.ID, lease, prepared)
	if _, err := store.DecideApprovalAndWakeRun(fixture21Identity("phase22-overwrite-approver", policy.RoleApprover), DecideApprovalInput{
		ApprovalID: prepared.ID, ProposalHash: prepared.ProposalHash, ExpectedVersion: prepared.Version,
		Decision: ApprovalStatusApproved,
	}); err != nil {
		t.Fatal(err)
	}
	claimed, ok, err := store.ClaimNextRun(context.Background(), ClaimInput{Owner: "worker-phase22-overwrite", LeaseDuration: time.Hour})
	if err != nil || !ok {
		t.Fatalf("claim approved Run: ok=%v err=%v", ok, err)
	}
	resumeCtx := fixture22OperatorContext(claimed.Run.UserID)
...
func TestTwoPhaseApprovalExpiryScanUsesDecisionPrimitiveAndTerminalInvalidatesPreparing(t *testing.T) {
	db := newP07Database(t, "phase22_expiry_terminal")
	store, ownerCtx, run, lease := fixture22RunningRun(t, db, "expiry")
	prepared := fixture22Prepare(t, store, ownerCtx, run, lease, "block_ip", policy.RiskL2, time.Now().Add(-time.Minute))
	fixture22Publish(t, store, ownerCtx, run.ID, lease, prepared)
	...
	terminalStore, terminalCtx, terminalRun, terminalLease := fixture22RunningRun(t, db, "terminal")
	terminalPreparing := fixture22Prepare(t, terminalStore, terminalCtx, terminalRun, terminalLease, "block_ip", policy.RiskL2, time.Now().Add(time.Hour))
...
func fixture22RunningRun(t *testing.T, db *gorm.DB, suffix string) (*GORMStore, context.Context, *mysql.WorkflowRun, LeaseToken) {
	t.Helper()
	store := NewGORMStore(db)
	ctx := fixture22OperatorContext("operator-phase22-" + suffix)
	run, err := store.CreateRunWithSessionLock(ctx, fixture08CreateInput("run-phase22-"+suffix, "session-phase22-"+suffix))
	if err != nil {
		t.Fatalf("create phase22 Run: %v", err)
	}
	...
	if err := db.First(run, "id = ?", run.ID).Error; err != nil {
		t.Fatalf("reload running phase22 Run: %v", err)
	}
	return store, ctx, run, lease
}

func fixture22Prepare(t *testing.T, store *GORMStore, ctx context.Context, run *mysql.WorkflowRun, lease LeaseToken, toolName string, risk policy.RiskLevel, expiresAt time.Time) *mysql.AgentApproval {
	t.Helper()
	proposalHash := fmt.Sprintf("%064x", len(run.ID)+len(toolName))
	approval, err := store.PrepareApproval(ctx, fixture22PrepareInput(run, lease, proposalHash, toolName, risk, expiresAt))
	if err != nil {
...
func fixture22PrepareInput(run *mysql.WorkflowRun, lease LeaseToken, proposalHash, toolName string, risk policy.RiskLevel, expiresAt time.Time) PrepareApprovalInput {
...
func fixture22Publish(t *testing.T, store *GORMStore, ctx context.Context, runID string, lease LeaseToken, approval *mysql.AgentApproval) {
	t.Helper()
...
func fixture22OperatorContext(userID string) context.Context {
