...
func TestRuntimeSnapshotPersistenceAndContextEnvelope(t *testing.T) {
	db := newP07Database(t, "phase11_snapshot_persistence")
	input := fixture08CreateInput("run-phase11", "session-phase11")
	...
	run, err := NewGORMStore(db).CreateRunWithSessionLock(fixture08UserContext("user-phase11"), input)
	if err != nil {
	...
	if stored.RuntimeVersion == nil || *stored.RuntimeVersion != input.RuntimeSnapshot.RuntimeVersion ||
		...
		stored.ModelSnapshot == nil || !fixture08JSONEqual(*stored.ModelSnapshot, string(input.RuntimeSnapshot.ModelSnapshotJSON)) ||
		stored.ToolSnapshot == nil || !fixture08JSONEqual(*stored.ToolSnapshot, string(input.RuntimeSnapshot.ToolSnapshotJSON)) ||
		stored.MCPCatalogHash == nil || *stored.MCPCatalogHash != input.RuntimeSnapshot.MCPCatalogHash ||
		stored.SkillSnapshot == nil || !fixture08JSONEqual(*stored.SkillSnapshot, string(input.RuntimeSnapshot.SkillSnapshotJSON)) ||
	...
	if contextSnapshot.Schema != DurableContextSnapshotSchema || contextSnapshot.Identity.UserID != "user-phase11" ||
		contextSnapshot.Identity.Role != string(policy.RoleViewer) || contextSnapshot.Identity.Scope.UserID != "user-phase11" ||
		!fixture08JSONEqual(string(contextSnapshot.BudgetLimits), string(input.BudgetLimitsJSON)) || !contextSnapshot.DeadlineAt.Equal(deadline) {
