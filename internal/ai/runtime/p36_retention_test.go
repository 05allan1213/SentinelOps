...
type p36RetentionBackend struct {
	mu       sync.Mutex
	claimed  bool
	cleanups int
}

func (b *p36RetentionBackend) ClaimRetentionLease(_ context.Context, _ workflow.RetentionClaimInput) (*workflow.RetentionLease, bool, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if b.claimed {
	...
	b.claimed = true
	return &workflow.RetentionLease{Token: workflow.LeaseToken{RunID: workflow.RetentionLeaseRunID, Owner: "worker-p36", Generation: 1}}, true, nil
}

func (b *p36RetentionBackend) ReleaseRetentionLease(context.Context, workflow.RetentionReleaseInput) error {
	return nil
}

func (b *p36RetentionBackend) PhysicalDeleteSensitivePayloads(context.Context, time.Time, int) (workflow.RetentionCleanupCounts, error) {
	b.mu.Lock()
	b.cleanups++
	b.mu.Unlock()
...
func (b *p36RetentionBackend) PhysicalDeleteAuditMetadata(context.Context, time.Time, int) (workflow.RetentionCleanupCounts, error) {
	return workflow.RetentionCleanupCounts{}, nil
}

type p36TraceRetention struct{ payloadCalls, auditCalls int }

func (d *p36TraceRetention) PhysicalDeleteTracePayloads(context.Context, time.Time, int) (int64, error) {
	d.payloadCalls++
	return 1, nil
}

func (d *p36TraceRetention) PhysicalDeleteTraceMetadata(context.Context, time.Time, int) (int64, error) {
	d.auditCalls++
	return 1, nil
}

func TestCleanupLeaseAllowsOnlyOneWorker(t *testing.T) {
	backend := &p36RetentionBackend{}
	traceDAO := &p36TraceRetention{}
	coordinator, err := NewRetentionCoordinator(backend, traceDAO, RetentionConfig{
		Owner: "worker-p36", LeaseDuration: time.Minute, Interval: time.Hour, BatchSize: 100,
...
func TestRetentionOnlyWorkerDoesNotClaimRuns(t *testing.T) {
	...
	worker, err := NewWorker(nil, WorkerConfig{
		Owner: "worker-p36", LeaseDuration: time.Minute,
