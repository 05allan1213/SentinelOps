...
func TestWorkerRunsAfterAPIContextIsGone(t *testing.T) {
	...
	}, Token: workflow.LeaseToken{RunID: "run-worker", Owner: "worker-p20", Generation: 1}}
...
func TestWorkerRetryableFailureUsesBoundedBackoffWithoutResettingAttempt(t *testing.T) {
	...
	}, Token: workflow.LeaseToken{RunID: "run-retry", Owner: "worker-p20", Generation: 4}}
...
func TestWorkerFatalFailureUsesOnlyCompletePrimitive(t *testing.T) {
	...
	}, Token: workflow.LeaseToken{RunID: "run-fatal", Owner: "worker-p20", Generation: 1}}
	var completions atomic.Int32
	worker, err := NewWorker(nil, WorkerConfig{
		Owner: "worker-p20", LeaseDuration: time.Minute,
...
func TestWorkerCanceledFailureUsesOnlyCompletePrimitive(t *testing.T) {
	...
	}, Token: workflow.LeaseToken{RunID: "run-canceled", Owner: "worker-p20", Generation: 1}}
...
func TestWorkerParkedFailureUsesFencedTransition(t *testing.T) {
	...
	}, Token: workflow.LeaseToken{RunID: "run-parked", Owner: "worker-p20", Generation: 1}}
	var transition workflow.RunTransition
	var completions atomic.Int32
	worker, err := NewWorker(nil, WorkerConfig{
		Owner: "worker-p20", LeaseDuration: time.Minute,
...
func TestWorkerLeaseLossCancelsExecutionAndSkipsTruthWrites(t *testing.T) {
	...
	}, Token: workflow.LeaseToken{RunID: "run-lease-loss", Owner: "worker-p20", Generation: 2}}
	var transitions, completions atomic.Int32
	worker, err := NewWorker(nil, WorkerConfig{
		Owner: "worker-p20", LeaseDuration: 3 * time.Millisecond,
		MinPollBackoff: time.Millisecond, MaxPollBackoff: 10 * time.Millisecond,
		ClaimNext: func(context.Context) (*workflow.ClaimedRun, bool, error) { return claimed, true, nil },
...
func TestWorkerPollLoopContinuesAfterLeaseLoss(t *testing.T) {
	...
	}, Token: workflow.LeaseToken{RunID: "run-loop-lease-loss", Owner: "worker-p20", Generation: 2}}
...
func TestAPIWorkerFocusedIntegration(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p20_api_worker")
	...
	ctx := policy.WithIdentity(context.Background(), policy.Identity{
		UserID: "user-p20", Role: policy.RoleOperator, Scope: policy.Scope{UserID: "user-p20"},
	})
	...
	run, err := store.CreateRunWithSessionLock(ctx, workflow.CreateRunInput{
		ID: "run-p20-focused", WorkflowKey: "chat.intent", SessionID: "session-p20-focused",
	...
	executor, err := NewDurableExecutor(store, func(context.Context, string) (adk.Agent, error) {
		return &p20FinalAgent{calls: &modelCalls}, nil
	}, frozen.CompatibilityHash())
	if err != nil {
		t.Fatal(err)
	}
	worker, err := NewWorker(store, WorkerConfig{
		Owner: "worker-p20-focused", LeaseDuration: 30 * time.Second,
		MinPollBackoff: time.Millisecond, MaxPollBackoff: 10 * time.Millisecond,
	...
	otherUser := policy.WithIdentity(context.Background(), policy.Identity{
		UserID: "other-user-p20", Role: policy.RoleOperator, Scope: policy.Scope{UserID: "other-user-p20"},
	})
	...
	if _, err := store.CreateRunWithSessionLock(ctx, workflow.CreateRunInput{
		ID: "run-p20-after-terminal", WorkflowKey: "chat.intent", SessionID: run.SessionID,
...
func TestWorkerRetryableBackoffPersistsAndReclaims(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p20_retry_backoff")
	store := workflow.NewGORMStore(db)
	frozen, err := FreezeRuntimeSnapshot(p11SnapshotInput())
	if err != nil {
		t.Fatal(err)
	...
	ctx := policy.WithIdentity(context.Background(), policy.Identity{
		UserID: "user-p20-retry", Role: policy.RoleOperator, Scope: policy.Scope{UserID: "user-p20-retry"},
	})
	run, err := store.CreateRunWithSessionLock(ctx, workflow.CreateRunInput{
		ID: "run-p20-retry", WorkflowKey: "chat.intent", SessionID: "session-p20-retry",
	...
	worker, err := NewWorker(store, WorkerConfig{
		Owner: "worker-p20-retry", LeaseDuration: time.Minute,
...
func TestWorkerSafePointCancelCommitsCheckpointForHandoff(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p20_safe_point")
	store := workflow.NewGORMStore(db)
	frozen, err := FreezeRuntimeSnapshot(p11SnapshotInput())
	if err != nil {
		t.Fatal(err)
	...
	ownerCtx := policy.WithIdentity(context.Background(), policy.Identity{
		UserID: "user-p20-drain", Role: policy.RoleOperator, Scope: policy.Scope{UserID: "user-p20-drain"},
	})
	run, err := store.CreateRunWithSessionLock(ownerCtx, workflow.CreateRunInput{
		ID: "run-p20-drain", WorkflowKey: "chat.intent", SessionID: "session-p20-drain",
	...
	executor, err := NewDurableExecutor(store, func(context.Context, string) (adk.Agent, error) {
		return &p20SafePointAgent{started: started, release: release}, nil
	}, frozen.CompatibilityHash())
	if err != nil {
		t.Fatal(err)
	}
	worker, err := NewWorker(store, WorkerConfig{
		Owner: "worker-p20-drain", LeaseDuration: time.Minute,
		MinPollBackoff: time.Millisecond, MaxPollBackoff: 10 * time.Millisecond,
		Execute: executor.ExecuteClaimedRun,
	})
	if err != nil {
...
func TestSessionRunParkedKeepsExclusiveLock(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p20_parked_lock")
	store := workflow.NewGORMStore(db)
	frozen, err := FreezeRuntimeSnapshot(p11SnapshotInput())
	if err != nil {
		t.Fatal(err)
	...
	ctx := policy.WithIdentity(context.Background(), policy.Identity{
		UserID: "user-p20-parked", Role: policy.RoleOperator, Scope: policy.Scope{UserID: "user-p20-parked"},
	})
	input := workflow.CreateRunInput{
		ID: "run-p20-parked", WorkflowKey: "chat.intent", SessionID: "session-p20-parked",
	...
	claimed, ok, err := store.ClaimNextRun(ctx, workflow.ClaimInput{Owner: "worker-p20-parked", LeaseDuration: time.Minute})
	if err != nil || !ok {
...
type p20FinalAgent struct{ calls *atomic.Int32 }

func (*p20FinalAgent) Name(context.Context) string        { return "P20L0Agent" }
func (*p20FinalAgent) Description(context.Context) string { return "P20 L0 scripted Agent" }
func (a *p20FinalAgent) Run(context.Context, *adk.AgentInput, ...adk.AgentRunOption) *adk.AsyncIterator[*adk.AgentEvent] {
	a.calls.Add(1)
	iter, generator := adk.NewAsyncIteratorPair[*adk.AgentEvent]()
	generator.Send(&adk.AgentEvent{AgentName: "P20L0Agent", Output: &adk.AgentOutput{MessageOutput: &adk.MessageVariant{Message: schema.AssistantMessage("focused answer", nil)}}})
	generator.Close()
	return iter
}

type p20SafePointAgent struct {
	started chan<- struct{}
	release <-chan struct{}
}

func (*p20SafePointAgent) Name(context.Context) string        { return "P20SafePointAgent" }
func (*p20SafePointAgent) Description(context.Context) string { return "P20 safe-point Agent" }
func (a *p20SafePointAgent) Run(ctx context.Context, _ *adk.AgentInput, _ ...adk.AgentRunOption) *adk.AsyncIterator[*adk.AgentEvent] {
	iter, generator := adk.NewAsyncIteratorPair[*adk.AgentEvent]()
	go func() {
		a.started <- struct{}{}
		<-a.release
		generator.Send(adk.StatefulInterrupt(ctx, "p20-safe-point", p12RegisteredRecoveryState{Marker: "p20"}))
		generator.Close()
	}()
	return iter
}
