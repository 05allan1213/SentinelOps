...
func TestTransactionalEffectApprovedResumeCallsOriginalEndpointOnce(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p23_approved_resume")
	store, ctx := p22RuntimeContext(t, db, "p23-approved-resume", "create_report", policy.RiskL1)
	p23InstallBudgetTruth(t, db, "run-p22-p23-approved-resume")
	chatModel := &p23CreateReportModel{}
	runner, attempt := p22NewRunnerHarnessWithModel(t, store, ctx, store, chatModel)
	...
	approverID := "approver-p23-runtime"
	...
	claimed, ok, err := store.ClaimNextRun(context.Background(), workflow.ClaimInput{
		Owner: "worker-p23-runtime-resume", LeaseDuration: time.Hour,
	})
	...
	resumeCtx, err = WithModelInvocation(resumeCtx, ModelInvocation{
		ReservationIdentity: "p23-resume-model", CatalogRef: modelSnapshot.CatalogRef,
...
func p23InstallBudgetTruth(t *testing.T, db *gorm.DB, runID string) {
	t.Helper()
	...
	if run.ContextSnapshotJSON == nil {
		t.Fatal("P23 fixture missing context snapshot")
	}
...
type p23CreateReportModel struct {
	calls int
}

func (m *p23CreateReportModel) Generate(context.Context, []*schema.Message, ...model.Option) (*schema.Message, error) {
	m.calls++
	if m.calls == 1 {
		return &schema.Message{Role: schema.Assistant, ToolCalls: []schema.ToolCall{{
			ID: "p23-create-report-call", Type: "function",
			Function: schema.FunctionCall{Name: "create_report", Arguments: `{"title":"P23 transactional report","content":"atomic","type":"custom"}`},
		}}}, nil
	}
	return schema.AssistantMessage("report persisted", nil), nil
}

func (m *p23CreateReportModel) Stream(ctx context.Context, input []*schema.Message, options ...model.Option) (*schema.StreamReader[*schema.Message], error) {
...
func (*p23CreateReportModel) BindTools([]*schema.ToolInfo) error { return nil }
