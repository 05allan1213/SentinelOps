...
func TestStatefulInterruptCoversAllToolWrappersBeforeMutationEndpoint(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p22_stateful_interrupt")
	store, ctx := p22RuntimeContext(t, db, "stateful-interrupt", "create_report", policy.RiskL1)
	...
	}, &adk.ToolContext{Name: "create_report", CallID: "p22-invokable"})
	if err != nil {
		t.Fatal(err)
	}
	...
	}, &adk.ToolContext{Name: "create_report", CallID: "p22-streamable"})
	if err != nil {
		t.Fatal(err)
	}
	...
	}, &adk.ToolContext{Name: "create_report", CallID: "p22-enhanced"})
	if err != nil {
		t.Fatal(err)
	}
	...
	}, &adk.ToolContext{Name: "create_report", CallID: "p22-enhanced-stream"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStatefulInterruptProductionHandlerAndClosedGateFailClosed(t *testing.T) {
	ctx, _ := p14InvocationContext(t, "run-p22-production-closed", "operator-p22-production", newP14RecordingBudget())
	endpointCalls := 0
...
func TestStatefulInterruptTerminalDecisionDoesNotReopenSameProposal(t *testing.T) {
	for _, terminal := range []struct {
		...
		t.Run(terminal.status, func(t *testing.T) {
			db := p12NewRuntimeDatabase(t, "p22_no_reopen_"+terminal.status)
			store, ctx := p22RuntimeContext(t, db, "no-reopen-"+terminal.status, "create_report", policy.RiskL1)
			handler, err := NewHITLRuntimeHandler(store)
			if err != nil {
				t.Fatal(err)
			}
			...
			if err := db.Model(&mysql.AgentApproval{}).Where("run_id = ?", "run-p22-no-reopen-"+terminal.status).Updates(map[string]any{
			...
			var count int64
			if err := db.Model(&mysql.AgentApproval{}).Where("run_id = ?", "run-p22-no-reopen-"+terminal.status).Count(&count).Error; err != nil {
				t.Fatal(err)
			}
...
func TestTwoPhaseApprovalRunnerPublishesOnlyAfterCheckpointEvent(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p22_runner_publish")
	store, ctx := p22RuntimeContext(t, db, "runner-publish", "create_report", policy.RiskL1)
	runner, attempt := p22NewRunnerHarness(t, store, ctx, store)
...
func TestTwoPhaseApprovalCheckpointSetFailureNeverPublishesPending(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p22_checkpoint_set_failure")
	store, ctx := p22RuntimeContext(t, db, "checkpoint-set-failure", "create_report", policy.RiskL1)
	checkpointFailure := errors.New("P22 injected CheckPointStore.Set failure")
	runner, attempt := p22NewRunnerHarness(t, store, ctx, &p22FailingCheckpointStore{delegate: store, setErr: checkpointFailure})
	execution, err := InvokeRecoveryRunner(ctx, runner, attempt, RecoveryDecision{
...
func TestTwoPhaseApprovalWorkerLoopExpiresDueApproval(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p22_worker_expiry")
	store, ctx := p22RuntimeContext(t, db, "worker-expiry", "create_report", policy.RiskL1)
	...
	worker, err := NewWorker(store, WorkerConfig{
		Owner: "worker-p22-expiry-loop", LeaseDuration: time.Minute,
...
func TestResumeAuthorizationIgnoresForgedDecisionAndDoesNotReinterruptTerminal(t *testing.T) {
	for _, decision := range []string{workflow.ApprovalStatusRejected, workflow.ApprovalStatusExpired} {
		t.Run(decision, func(t *testing.T) {
			db := p12NewRuntimeDatabase(t, "p22_forged_resume_"+decision)
			store, ctx := p22RuntimeContext(t, db, "forged-resume-"+decision, "create_report", policy.RiskL1)
			...
			observingModel := &p22TerminalObservingModel{expected: terminalCode}
			runner, attempt := p22NewRunnerHarnessWithModel(t, store, ctx, store, observingModel)
			execution, err := InvokeRecoveryRunner(ctx, runner, attempt, RecoveryDecision{
			...
			approverID := "approver-p22-" + decision
			...
			claimed, ok, err := store.ClaimNextRun(context.Background(), workflow.ClaimInput{Owner: "worker-p22-forged-" + decision, LeaseDuration: time.Hour})
			if err != nil || !ok {
			...
			resumeCtx, err = WithModelInvocation(resumeCtx, ModelInvocation{
				ReservationIdentity: "p22-forged-model-" + decision, CatalogRef: modelSnapshot.CatalogRef,
...
func TestCompositeInterruptPropagatesApprovalThroughAgentTool(t *testing.T) {
	child := &p22ApprovalInterruptAgent{}
...
type p22ApprovalInterruptAgent struct{}

func (*p22ApprovalInterruptAgent) Name(context.Context) string { return "p22_approval_child" }
func (*p22ApprovalInterruptAgent) Description(context.Context) string {
	return "P22 nested Approval child"
}
func (*p22ApprovalInterruptAgent) Run(ctx context.Context, _ *adk.AgentInput, _ ...adk.AgentRunOption) *adk.AsyncIterator[*adk.AgentEvent] {
...
func p22RuntimeContext(t *testing.T, db *gorm.DB, suffix, toolName string, risk policy.RiskLevel, additionalTools ...string) (*workflow.GORMStore, context.Context) {
	...
	if err != nil || entry.Risk != risk {
		t.Fatalf("P22 Catalog fixture entry=%#v err=%v", entry, err)
	}
	...
	runID := "run-p22-" + suffix
	run, err := store.CreateRunWithSessionLock(identityContext, workflow.CreateRunInput{
		ID: runID, WorkflowKey: "p22-runtime-test", SessionID: "session-p22-" + suffix,
		QueryText: "P22 HITL", ImmutableInputJSON: json.RawMessage(`{"agent":"test","query":"P22 HITL"}`),
	...
	claimed, ok, err := store.ClaimNextRun(context.Background(), workflow.ClaimInput{Owner: "worker-p22-" + suffix, LeaseDuration: time.Hour})
	if err != nil || !ok || claimed.Run.ID != run.ID {
		t.Fatalf("claim P22 Runtime Run: claimed=%#v ok=%v err=%v", claimed, ok, err)
	}
	...
	attempt := &AttemptContext{
		...
		Trace: TraceIdentity{ID: "trace-p22-" + suffix}, Deadline: time.Now().Add(time.Hour), Snapshot: frozen,
	}
	...
	ctx, err = WithModelInvocation(ctx, ModelInvocation{
		ReservationIdentity: "p22-model-" + suffix, CatalogRef: modelSnapshot.CatalogRef,
		Provider: modelSnapshot.Provider, Driver: modelSnapshot.Driver, ModelID: modelSnapshot.ModelID,
		Profile: modelSnapshot.Profile, SnapshotIdentity: modelSnapshot.Identity(),
	})
	if err != nil {
...
func p22NewRunnerHarness(t *testing.T, store *workflow.GORMStore, ctx context.Context, checkpointStore adk.CheckPointStore) (*adk.Runner, *AttemptContext) {
	t.Helper()
	return p22NewRunnerHarnessWithModel(t, store, ctx, checkpointStore, &p14MutationToolCallingModel{})
}

func p22NewRunnerHarnessWithModel(
	t *testing.T,
	...
	agent, err := adk.NewChatModelAgent(ctx, &adk.ChatModelAgentConfig{
		Name: "p22_runner_agent", Description: "P22 Runner publish contract", Model: chatModel,
...
type p22FailingCheckpointStore struct {
	delegate adk.CheckPointStore
	setErr   error
}

func (s *p22FailingCheckpointStore) Get(ctx context.Context, checkpointID string) ([]byte, bool, error) {
	return s.delegate.Get(ctx, checkpointID)
}

func (s *p22FailingCheckpointStore) Set(context.Context, string, []byte) error {
	return s.setErr
}

type p22TerminalObservingModel struct {
	expected    string
	calls       atomic.Int32
...
func (m *p22TerminalObservingModel) Generate(_ context.Context, input []*schema.Message, _ ...model.Option) (*schema.Message, error) {
	if m.calls.Add(1) == 1 {
		return &schema.Message{Role: schema.Assistant, ToolCalls: []schema.ToolCall{{
			ID: "p22-forged-resume-call", Type: "function",
...
func (m *p22TerminalObservingModel) Stream(ctx context.Context, input []*schema.Message, options ...model.Option) (*schema.StreamReader[*schema.Message], error) {
...
func (*p22TerminalObservingModel) BindTools([]*schema.ToolInfo) error { return nil }

func (a *p22ApprovalInterruptAgent) String() string { return fmt.Sprintf("%T", a) }
