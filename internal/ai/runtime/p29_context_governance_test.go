...
func TestRedisProjectionFailureDoesNotChangeMySQLCompletion(t *testing.T) {
	...
	worker, err := NewWorker(nil, WorkerConfig{
		Owner: "p29-worker", LeaseDuration: time.Minute, MinPollBackoff: time.Millisecond, MaxPollBackoff: time.Second,
		ClaimNext: func(context.Context) (*workflow.ClaimedRun, bool, error) {
			return &workflow.ClaimedRun{Run: mysql.WorkflowRun{ID: "run-p29", Status: workflow.RunStatusRunning}, Token: workflow.LeaseToken{RunID: "run-p29", Owner: "p29-worker", Generation: 1}}, true, nil
		},
