...
func TestEffectiveGateUsesExactCanonicalKeysAndFailsClosed(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "all true", values: fixture42DynamicValues("true"), gate: GateMCPEnabled, want: true},
		{name: "explicit false", values: fixture42DynamicValues("true"), gate: GateSkillEnabled, want: false},
		{name: "missing", values: fixture42DynamicValues("true"), gate: GateLangfuseEnabled, want: false},
		{name: "illegal", values: fixture42DynamicValues("true"), gate: GateAgentRuntimeEnabled, want: false},
		{name: "unknown alias", values: fixture42DynamicValues("true"), gate: GateAgentRuntimeAcceptNewRuns, want: false},
	}
...
func TestEffectiveGateTruthTableForEveryCanonicalKey(t *testing.T) {
	for _, key := range CanonicalGateKeys() {
		t.Run(key, func(t *testing.T) {
			baseline := fixture42GateBools(true)
...
func TestEffectiveGateCurrentStateUsesOneDynamicRead(t *testing.T) {
	...
	evaluator, err := NewGateEvaluator(allOpenP42StaticCaps(), func(context.Context, []string) (map[string]string, error) {
		reads++
		return fixture42DynamicValues("true"), nil
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestShadowModeClosesL1AndL2Writes(t *testing.T) {
	values := fixture42DynamicValues("true")
	evaluator, err := NewGateEvaluator(allOpenP42StaticCaps(), func(context.Context, []string) (map[string]string, error) {
		return values, nil
	})
	if err != nil {
...
func TestLegacyCompatibilityGateRequiresEnabledAndShadow(t *testing.T) {
	cases := []struct {
		...
		{name: "enabled shadow", staticEnabled: true, staticShadow: true, dynamic: fixture42DynamicValues("true"), want: true},
		{name: "enabled closed", staticEnabled: true, staticShadow: true, dynamic: fixture42DynamicValues("true")},
		{name: "shadow closed", staticEnabled: true, staticShadow: true, dynamic: fixture42DynamicValues("true")},
		{name: "static enabled closed", staticShadow: true, dynamic: fixture42DynamicValues("true")},
		{name: "static shadow closed", staticEnabled: true, dynamic: fixture42DynamicValues("true")},
		{name: "missing", staticEnabled: true, staticShadow: true, dynamic: fixture42DynamicValues("true")},
		{name: "illegal", staticEnabled: true, staticShadow: true, dynamic: fixture42DynamicValues("true")},
		{name: "read error", staticEnabled: true, staticShadow: true, dynamic: fixture42DynamicValues("true"), readErr: errors.New("read failed")},
	}
...
func TestRollbackCompatibilityCurrentStaticGateDoesNotRewriteFrozenHash(t *testing.T) {
	...
	frozenGates := fixture42GateBools(false)
...
func TestRollbackCompatibilityWorkerClaimsExactRuntimeVersion(t *testing.T) {
	db := fixture12NewRuntimeDatabase(t, "phase42_runtime_version")
	store := workflow.NewGORMStore(db)
	identity := policy.Identity{UserID: "admin-phase42-version", Role: policy.RoleAdmin, Scope: policy.Scope{UserID: "admin-phase42-version"}}
	ctx := policy.WithIdentity(context.Background(), identity)

	mismatchInput := fixture11SnapshotInput()
	mismatchInput.Runtime.App = "compatibility-image"
	mismatch, err := FreezeRuntimeSnapshot(mismatchInput)
	if err != nil {
		t.Fatal(err)
	}
	currentInput := fixture11SnapshotInput()
	...
	for _, item := range []struct {
		...
		{id: "a-mismatch-phase42", session: "session-mismatch-phase42", snapshot: mismatch},
		{id: "b-current-phase42", session: "session-current-phase42", snapshot: current},
	} {
		if _, err := store.CreateRunWithSessionLock(ctx, workflow.CreateRunInput{
			ID: item.id, WorkflowKey: "phase42.runtime-version", SessionID: item.session,
	...
	claimed, ok, err := store.ClaimNextRun(context.Background(), workflow.ClaimInput{
		Owner: "worker-current-phase42", LeaseDuration: time.Minute, RuntimeVersion: current.RuntimeVersion(),
	})
	if err != nil || !ok || claimed.Run.ID != "b-current-phase42" {
	...
	if other, ok, err := store.ClaimNextRun(context.Background(), workflow.ClaimInput{
		Owner: "worker-unknown-phase42", LeaseDuration: time.Minute, RuntimeVersion: `{"app":"unknown","eino":"v0","go":"go0"}`,
...
func TestFrozenSnapshotExistingRunStillClaimsWhenAcceptNewRunsCloses(t *testing.T) {
	db := fixture12NewRuntimeDatabase(t, "phase42_accept_closed_existing")
	store := workflow.NewGORMStore(db)
	identity := policy.Identity{UserID: "admin-phase42-existing", Role: policy.RoleAdmin, Scope: policy.Scope{UserID: "admin-phase42-existing"}}
	ctx := policy.WithIdentity(context.Background(), identity)
	input := fixture11SnapshotInput()
	...
	if _, err := store.CreateRunWithSessionLock(ctx, workflow.CreateRunInput{
		ID: "existing-phase42", WorkflowKey: "phase42.accept-closed", SessionID: "session-existing-phase42",
	...
	dynamic := fixture42DynamicValues("true")
	...
	worker, err := NewWorker(store, WorkerConfig{
		Owner: "worker-existing-phase42", LeaseDuration: time.Minute,
	...
	if err != nil || !ok || claimed.Run.ID != "existing-phase42" {
...
func TestEffectiveGateRuntimeDisabledSkipsEffectReconciliationClaim(t *testing.T) {
	db := fixture12NewRuntimeDatabase(t, "phase42_runtime_disabled_reconciliation")
	store, runCtx, runID, _ := fixture25UnknownEffectFixture(t, db, "phase42-runtime-disabled", policy.EffectReconcilable)
	...
	queryCalls := 0
	worker, err := NewWorker(store, WorkerConfig{
		Owner: "worker-phase42-runtime-disabled", LeaseDuration: time.Minute,
		MinPollBackoff: time.Millisecond, MaxPollBackoff: 10 * time.Millisecond,
...
func fixture42DynamicValues(value string) map[string]string {
...
func fixture42GateBools(value bool) map[string]bool {
