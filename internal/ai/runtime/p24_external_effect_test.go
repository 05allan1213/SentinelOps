...
func TestExternalEffectApprovedResumeUsesLedgerDeadlineAndIdempotency(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		writer.Header().Set("X-Request-ID", "request-p24-runtime")
	...
	db := p12NewRuntimeDatabase(t, "p24_external_resume")
	store, ctx := p22RuntimeContext(t, db, "p24-external-resume", "webhook_out", policy.RiskL2)
	p23InstallBudgetTruth(t, db, "run-p22-p24-external-resume")
	chatModel := &p24WebhookModel{url: server.URL}
	runner, attempt := p24NewExternalRunnerHarness(t, store, ctx, chatModel)
	...
	approverID := "approver-p24-runtime"
	...
	claimed, ok, err := store.ClaimNextRun(context.Background(), workflow.ClaimInput{
		Owner: "worker-p24-runtime-resume", LeaseDuration: time.Hour,
	})
	...
	resumeCtx, err = WithModelInvocation(resumeCtx, ModelInvocation{
		ReservationIdentity: "p24-resume-model", CatalogRef: modelSnapshot.CatalogRef,
	...
	if calls.Load() != 1 || key == "" || key != effect.ID || effect.Status != workflow.EffectStatusSucceeded ||
		effect.ExternalReference == nil || *effect.ExternalReference != "request-p24-runtime" {
		t.Fatalf("calls=%d key=%q effect=%#v", calls.Load(), key, effect)
	}
}

func p24NewExternalRunnerHarness(t *testing.T, store *workflow.GORMStore, ctx context.Context, chatModel model.BaseChatModel) (*adk.Runner, *AttemptContext) {
	t.Helper()
	...
	agent, err := adk.NewChatModelAgent(ctx, &adk.ChatModelAgentConfig{
		Name: "p24_external_agent", Description: "P24 external Effect contract", Model: chatModel,
...
type p24WebhookModel struct {
	url   string
	calls int
}

func (m *p24WebhookModel) Generate(context.Context, []*schema.Message, ...model.Option) (*schema.Message, error) {
	m.calls++
	if m.calls == 1 {
		return &schema.Message{Role: schema.Assistant, ToolCalls: []schema.ToolCall{{
			ID: "p24-webhook-call", Type: "function", Function: schema.FunctionCall{
...
func (m *p24WebhookModel) Stream(ctx context.Context, input []*schema.Message, options ...model.Option) (*schema.StreamReader[*schema.Message], error) {
...
func (*p24WebhookModel) BindTools([]*schema.ToolInfo) error { return nil }
