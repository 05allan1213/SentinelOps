...
func TestMutationRouteSnapshotCoversEveryDurableMutation(t *testing.T) {
	...
	if frozen.FeatureGate("agent_runtime.l1_writes") || frozen.FeatureGate("agent_runtime.l2_writes") {
		t.Fatalf("P26 production Mutation gates must remain false: %+v", frozen.document.FeatureGates)
	}
}
...
func TestMutationRouteEveryLeafInterruptsBeforeEndpoint(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p26_every_leaf_interrupt")
	cases := []struct {
		name string
		risk policy.RiskLevel
		args string
	}{
		{name: "create_report", risk: policy.RiskL1, args: `{"title":"p26","content":"p26"}`},
		{name: "save_intelligence", risk: policy.RiskL1, args: `{"title":"p26","content":"p26"}`},
		{name: "update_event_status", risk: policy.RiskL1, args: `{"event_id":"p26","status":"resolved"}`},
		{name: "block_ip", risk: policy.RiskL2, args: `{"ip":"192.0.2.26"}`},
		{name: "notify_dingtalk", risk: policy.RiskL2, args: `{"title":"p26","content":"p26"}`},
		{name: "notify_wecom", risk: policy.RiskL2, args: `{"content":"p26"}`},
		{name: "notify_email", risk: policy.RiskL2, args: `{"subject":"p26"}`},
		{name: "webhook_out", risk: policy.RiskL2, args: `{"url":"https://example.invalid","payload":"{}"}`},
	}
	endpointCalls := 0
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			store, ctx := p22RuntimeContext(t, db, "p26-"+test.name, test.name, test.risk)
			...
			}, &adk.ToolContext{Name: test.name, CallID: "p26-" + test.name})
			if err != nil {
				t.Fatal(err)
			}
			...
			if err := db.Model(&mysql.AgentApproval{}).Where("run_id = ?", "run-p22-p26-"+test.name).Count(&approvalCount).Error; err != nil {
				t.Fatal(err)
			}
			if err := db.Model(&mysql.AgentEffect{}).Where("run_id = ?", "run-p22-p26-"+test.name).Count(&effectCount).Error; err != nil {
				t.Fatal(err)
			}
...
func contextWithAttemptForP26(t *testing.T) context.Context {
	t.Helper()
	return context.WithValue(context.Background(), attemptContextKey{}, &AttemptContext{Run: RunIdentity{ID: "run-p26"}})
}
