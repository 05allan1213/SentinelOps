...
func TestRuntimeSnapshotCompatibilityStableAndSensitive(t *testing.T) {
	...
	tests := []struct {
		...
		{"prompt", func(v *RuntimeSnapshotInput) { v.PromptHash = fixture11Hash("prompt-2") }},
		...
		{"tool_schema", func(v *RuntimeSnapshotInput) { v.Tools[0].SchemaHash = fixture11Hash("schema-2") }},
		{"mcp", func(v *RuntimeSnapshotInput) { v.MCPCatalogHash = fixture11Hash("mcp-2") }},
		{"skill", func(v *RuntimeSnapshotInput) { v.Skills[0].ContentHash = fixture11Hash("skill-2") }},
		{"gate", func(v *RuntimeSnapshotInput) { v.FeatureGates["agent_runtime.enabled"] = false }},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			changed := fixture11SnapshotInput()
			test.mutate(&changed)
...
func TestTypedContextRebuildsAttemptFromDatabaseTruth(t *testing.T) {
	...
	run := mysql.WorkflowRun{
		...
		Status: workflow.RunStatusRunning, Attempt: 1, LeaseGeneration: 7, LeaseOwner: fixture11String("worker-1"),
		ContextSnapshotJSON: fixture11String(string(contextPayload)), RuntimeVersion: &fields.RuntimeVersion,
		RuntimeCompatibilityHash: &fields.RuntimeCompatibilityHash, AgentRevision: &fields.AgentRevision,
		ModelSnapshot: fixture11String(string(fields.ModelSnapshotJSON)), ToolSnapshot: fixture11String(string(fields.ToolSnapshotJSON)),
		MCPCatalogHash: &fields.MCPCatalogHash, SkillSnapshot: fixture11String(string(fields.SkillSnapshotJSON)),
		PromptHash: &fields.PromptHash, PolicyHash: &fields.PolicyHash, ConfigHash: &fields.ConfigHash,
		FeatureSnapshot: fixture11String(string(fields.FeatureSnapshotJSON)), BudgetLimitsJSON: fixture11String(`{"max_tokens":4096}`),
		BudgetUsageJSON: fixture11String(`{}`), BudgetReservationsJSON: fixture11String(`{}`),
	}
	claimed := workflow.ClaimedRun{Run: run, Token: workflow.LeaseToken{RunID: run.ID, Owner: "worker-1", Generation: 7}}
	budgetFactory := &fixture11BudgetFactory{}
	...
	if token, err := workflow.LeaseTokenFromContext(ctx); err != nil || token != claimed.Token {
		t.Fatalf("phase09 lease accessor = %+v, %v", token, err)
	}
...
func TestTraceAttemptPreservesRunSnapshotAndRotatesTraceID(t *testing.T) {
	baseRun, frozen := fixture11AttemptRun(t)
	...
	baseRun.LeaseOwner = fixture11String("worker-b")
...
func TestSessionValuesAllowlistRejectsHandlesSecretsAndMutableValues(t *testing.T) {
	valid := map[string]any{
		...
		SessionCompatibilityHashKey: fixture11Hash("compat"),
	}
...
type fixture11BudgetHandle struct{ id string }

func (fixture11BudgetHandle) RuntimeBudgetHandle() {}

type fixture11BudgetFactory struct{ state BudgetState }

func (f *fixture11BudgetFactory) RebuildBudgetHandle(_ context.Context, state BudgetState) (BudgetHandle, error) {
	f.state = state
	return fixture11BudgetHandle{id: "budget/" + state.RunID}, nil
}

func fixture11SnapshotInput() RuntimeSnapshotInput {
	thinking := false
	return RuntimeSnapshotInput{
		...
		PromptHash:    fixture11Hash("prompt-1"),
		PolicyHash:    fixture11Hash("policy-1"),
		ConfigHash:    fixture11Hash("config-1"),
		...
		Tools: []ToolSnapshot{
			{Name: "query_events", Revision: "tool-rev-1", SchemaHash: fixture11Hash("query-events-schema")},
			{Name: "search_similar_events", Revision: "tool-rev-1", SchemaHash: fixture11Hash("search-schema")},
		},
		MCPCatalogHash: fixture11Hash("mcp-1"),
		Skills:         []SkillSnapshot{{Name: "incident-analysis", ContentHash: fixture11Hash("skill-1")}},
...
func fixture11AttemptRun(t *testing.T) (mysql.WorkflowRun, FrozenRuntimeSnapshot) {
	t.Helper()
	frozen, err := FreezeRuntimeSnapshot(fixture11SnapshotInput())
	if err != nil {
		t.Fatal(err)
	...
	return mysql.WorkflowRun{
		...
		Status: workflow.RunStatusRunning, Attempt: 1, LeaseGeneration: 1, LeaseOwner: fixture11String("worker-a"),
		ContextSnapshotJSON: fixture11String(string(contextPayload)), RuntimeVersion: &fields.RuntimeVersion,
		RuntimeCompatibilityHash: &fields.RuntimeCompatibilityHash, AgentRevision: &fields.AgentRevision,
		ModelSnapshot: fixture11String(string(fields.ModelSnapshotJSON)), ToolSnapshot: fixture11String(string(fields.ToolSnapshotJSON)),
		MCPCatalogHash: &fields.MCPCatalogHash, SkillSnapshot: fixture11String(string(fields.SkillSnapshotJSON)), PromptHash: &fields.PromptHash,
		PolicyHash: &fields.PolicyHash, ConfigHash: &fields.ConfigHash, FeatureSnapshot: fixture11String(string(fields.FeatureSnapshotJSON)),
		BudgetLimitsJSON: fixture11String(`{}`), BudgetUsageJSON: fixture11String(`{}`), BudgetReservationsJSON: fixture11String(`{}`),
	}, frozen
}

func fixture11Hash(value string) string {
	digest := sha256.Sum256([]byte(value))
	return hex.EncodeToString(digest[:])
}

func fixture11String(value string) *string { return &value }
