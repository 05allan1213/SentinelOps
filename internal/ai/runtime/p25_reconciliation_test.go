...
func TestReconciliationWorkerAutomaticallyResolvesTargetState(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			db := p12NewRuntimeDatabase(t, "p25_worker_"+tc.name)
			store, runCtx, runID, runAttempt := p25UnknownEffectFixture(t, db, tc.name, policy.EffectReconcilable)
			queryCalls := 0
			worker, err := NewWorker(store, WorkerConfig{
				Owner: "worker-p25-" + tc.name, LeaseDuration: time.Minute,
...
func TestReconciliationWorkerNeverClaimsNonReconcilableEffect(t *testing.T) {
	db := p12NewRuntimeDatabase(t, "p25_worker_nonreconcilable")
	store, runCtx, runID, _ := p25UnknownEffectFixture(t, db, "nonreconcilable", policy.EffectNonReconciliableExternal)
	queryCalls := 0
	worker, err := NewWorker(store, WorkerConfig{
		Owner: "worker-p25-nonreconcilable", LeaseDuration: time.Minute,
		MinPollBackoff: time.Millisecond, MaxPollBackoff: 10 * time.Millisecond,
		ClaimNext: func(context.Context) (*workflow.ClaimedRun, bool, error) { return nil, false, nil },
		Execute: func(context.Context, *workflow.ClaimedRun) (RunExecutionResult, error) {
			return RunExecutionResult{}, nil
...
func p25UnknownEffectFixture(t *testing.T, db *gorm.DB, suffix string, effectType policy.EffectType) (*workflow.GORMStore, context.Context, string, uint) {
	t.Helper()
	toolName := "block_ip"
	request := `{"ip":"192.0.2.25","reason":"P25 reconciliation"}`
	if effectType == policy.EffectNonReconciliableExternal {
		toolName = "notify_email"
		request = `{"to":"ops@example.test","subject":"P25","body":"reconciliation"}`
	}
	store, ctx := p22RuntimeContext(t, db, "p25-"+suffix, toolName, policy.RiskL2)
