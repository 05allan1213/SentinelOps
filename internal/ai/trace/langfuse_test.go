...
type fixture36CountingExporter struct {
	inner    *tracetest.InMemoryExporter
	shutdown atomic.Int32
}

type fixture36RoundTripFunc func(*http.Request) (*http.Response, error)

func (fn fixture36RoundTripFunc) RoundTrip(request *http.Request) (*http.Response, error) {
	return fn(request)
}

func (e *fixture36CountingExporter) ExportSpans(ctx context.Context, spans []sdktrace.ReadOnlySpan) error {
	return e.inner.ExportSpans(ctx, spans)
}

func (e *fixture36CountingExporter) Shutdown(ctx context.Context) error {
	e.shutdown.Add(1)
...
func TestLangfuseGateDisabledMakesZeroNetworkCalls(t *testing.T) {
	...
	runtime, err := NewLangfuseRuntime(context.Background(), LangfuseOptions{
		...
		HTTPClient: &http.Client{Transport: fixture36RoundTripFunc(func(*http.Request) (*http.Response, error) {
			calls.Add(1)
...
func TestLangfuseOfficialLifecycleMetadataUsageAndRedaction(t *testing.T) {
	...
	exporter := &fixture36CountingExporter{inner: tracetest.NewInMemoryExporter()}
	runtime, err := NewLangfuseRuntime(context.Background(), LangfuseOptions{
		StaticEnabled:           true,
		DynamicEnabled:          true,
	...
	ctx := runtime.StartAttempt(context.Background(), AttemptMetadata{
		TraceID: "mysql-trace-phase36", RunID: "run-phase36", SessionID: "session-phase36",
	...
	for _, want := range []string{
		"langfuse.trace.metadata.run_id=run-phase36",
		"langfuse.trace.metadata.mysql_trace_id=mysql-trace-phase36",
