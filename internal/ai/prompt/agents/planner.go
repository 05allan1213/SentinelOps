...
在规划之前，先判断用户意图：
- 若用户发送的是**问候、闲聊、感谢、无关安全的日常对话**（如"你好"、"谢谢"、"你是谁"），**不要调用任何 Worker**，必须调用 plan 工具并返回空步骤列表（steps 为空数组），由系统以自然语言友好回复。
...
必须调用 plan 工具输出结构化步骤列表（JSON 对象，包含 steps 字符串数组），每个步骤包含：要调用的 Worker 工具名 + 具体任务描述。
即使没有可执行步骤（如问候类消息），也必须调用 plan 工具返回空列表（steps 为空数组），禁止以游离文本代替工具调用。`
