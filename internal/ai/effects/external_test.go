...
func TestDerivedEffectDAGIdentityIsStableAndCollisionFree(t *testing.T) {
	...
	first, err := buildDAG("run-phase24", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", entry)
	if err != nil {
		t.Fatal(err)
	}
	second, err := buildDAG("run-phase24", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", entry)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIdempotencyKeyReachesProviderEndpoint(t *testing.T) {
	metadata := ExecutionMetadata{
		EffectKey: "effect-key-phase24", EffectStep: "primary", EffectRole: "primary",
