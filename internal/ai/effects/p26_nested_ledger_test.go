...
func TestNoDirectWriteMutationEndpointRequiresEffectOrLegacyPermit(t *testing.T) {
	...
	ctx := withExecutionMetadata(context.Background(), ExecutionMetadata{
		EffectKey: "p26-effect", EffectStep: "primary", EffectRole: "primary", EffectType: policy.EffectTransactionalDB,
	})
...
func TestNestedLedgerOnlyLeafMutationOwnsEffectPlan(t *testing.T) {
	...
	dag, err := buildDAG("run-p26", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", leaf)
	if err != nil {
		t.Fatal(err)
	}
	primary := 0
