...
func TestRetentionAuditIsAtomicQueryableAndRedacted(t *testing.T) {
	_, db, dsn := newDisposableDatabase(t, "p36_retention_audit")
	...
	if err := UpdateRetentionPolicyWithAudit(ctx, RetentionPolicyChange{
		ActorID: "admin-p36", OldPayloadDays: 30, OldAuditDays: 180,
	...
	if len(records) != 1 || records[0].ActorID != "admin-p36" || records[0].OldPayloadDays != 30 || records[0].NewAuditDays != 240 {
