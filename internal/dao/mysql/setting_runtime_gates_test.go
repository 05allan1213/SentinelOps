...
func TestEffectiveGateAuditIsAtomicQueryableAndRedacted(t *testing.T) {
	_, db, dsn := newDisposableDatabase(t, "phase42_runtime_gate_audit")
	...
	change, err := UpdateRuntimeGatesWithAudit(ctx, keys, values, "admin-phase42", "ticket api_key="+secret)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(records) != 1 || records[0].ActorID != "admin-phase42" || records[0].ChangedAt.IsZero() {
...
func TestEffectiveGateAuditFailureRollsBackWholeVector(t *testing.T) {
	_, db, dsn := newDisposableDatabase(t, "phase42_runtime_gate_rollback")
	requireMigrationsUp(t, dsn)
	previousDB, previousErr := globalDB, initErr
	globalDB, initErr = db, nil
	t.Cleanup(func() {
	...
	ctx := context.Background()
	SeedRuntimeGateSettings(ctx, keys)
	if err := db.Exec(`CREATE TRIGGER phase42_reject_gate_audit BEFORE INSERT ON settings FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'reject phase42 audit'`).Error; err != nil {
		t.Fatal(err)
	}
	values := make(map[string]bool, len(keys))
	for _, key := range keys {
	...
	if _, err := UpdateRuntimeGatesWithAudit(ctx, keys, values, "admin-phase42", "atomic rollback"); err == nil {
