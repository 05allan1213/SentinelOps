...
func TestPhysicalDeleteTracePayloadUsesTerminalRunAnchor(t *testing.T) {
	_, db, dsn := newDisposableDatabase(t, "phase36_trace_retention")
	requireMigrationsUp(t, dsn)
	now := time.Now().UTC().Truncate(time.Millisecond)
	old := now.Add(-181 * 24 * time.Hour)
	terminalFinished := old
	terminal := WorkflowRun{
		ID: "run-phase36-trace-terminal", WorkflowKey: "phase36", UserID: "user-phase36", SessionID: "session-terminal",
	...
	parked := WorkflowRun{
		ID: "run-phase36-trace-parked", WorkflowKey: "phase36", UserID: "user-phase36", SessionID: "session-parked",
	...
	for _, fixture := range []struct{ traceID, runID string }{
		{"trace-phase36-terminal", terminal.ID}, {"trace-phase36-parked", parked.ID},
	} {
		tags := `{"run_id":"` + fixture.runID + `","trace_quality":"complete"}`
		if err := db.Create(&TraceRun{
			TraceID: fixture.traceID, TraceName: "durable.attempt", SessionID: "session-phase36",
	...
	if err := db.First(&terminalNode, "trace_id = ?", "trace-phase36-terminal").Error; err != nil {
		t.Fatal(err)
	}
	if err := db.First(&parkedNode, "trace_id = ?", "trace-phase36-parked").Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Model(&TraceRun{}).Where("trace_id = ?", "trace-phase36-parked").Count(&parkedCount).Error; err != nil || parkedCount != 1 {
