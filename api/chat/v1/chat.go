...
type ChatReq struct {
	...
	UserID       string `json:"user_id"`       // 兼容旧客户端；服务端忽略此字段并只使用 JWT Identity
	...
	RunID        string `json:"run_id"`        // 已有 durable Run ID；为空时才允许首次创建
	LastSeq      int64  `json:"last_seq"`      // 已收到的最后事件 seq；仅随 run_id 用作 v2 after_seq 游标
}
