...
- Long verification: the full `internal/ai/workflow` package did not complete in the 10-minute default timeout: `TestRecoveryLegalityMatrix` was still waiting on Goose disposable-database initialization (the same environment/migration bottleneck recorded in the B1 progress report), so that package run is classified NOT RUN TO COMPLETION rather than PASS.
